...
\item Finally, we evaluate the noncenter integral involving $Z$. To get the bound we want, as in \cite{Sandstede1998}, we will use a piecewise bound for $Z_i^\pm$. To obtain this, we substitute $Z_i^\pm$ into the RHS of the fixed point equations \cref{Zfpeq} and bound the integral as in \cref{L1boundlemma} to get
\begin{align*}
|Z_i^+(x)| \leq C \Big( e^{-\alpha X_*} \| Z_i^+\| + e^{-\tilde{\alpha}(X_i - x)}|a_i^+| + |b_i^+| + |c_i| + |\tilde{c}_i| + e^{-\alpha X_*}|\lambda|^2 |d| \Big)
\end{align*}
Substituting the bounds from Lemma \ref{Zinv2}, we have the piecewise bound
\begin{align*}
|Z_i^+(x)| \leq C \Big( e^{-\alpha(X_i - x)}|D_i||d| + |c_i| + (e^{-\alpha X_*} + |\lambda|)^2 |d| \Big)
\end{align*}
As on \cite[p. 448]{Sandstede1998}, since $DF(0)$ is hyperbolic, we can increase the decay constant $\alpha$ to $\alpha_0$ for $\Psi(x)$ to get the decay rate
\[
...
&\qquad \leq C \left( \int_0^{X_i} e^{-\alpha_0 y} e^{-\alpha X_i} e^{-\alpha(X_i - y)}|Z_i^+(y)|dy + \| Z_i^+\|  \int_0^{X_i} e^{-2 \alpha X_{i-1}} e^{-\alpha y} \right) dy
\end{align*}
For the second integral, we use the uniform bound $Z_3$ to get 
\begin{align*}
\| Z_i^+\| \int_0^{X_i} e^{-2 \alpha_0 X_{i-1}} e^{-\alpha_0 y} dy &\leq C e^{-2 \alpha X_* }\left( |c_i| + (e^{-\alpha X_*} + |\lambda|)^2 |d| + |D_i||d| \right)
\end{align*}
For the first integral, we have
\begin{align*}
\int_0^{X_i} &e^{-\alpha_0 y} e^{-\alpha X_i} e^{-\alpha(X_i - y)}|Z_i^+(y)|dy \\
&\leq e^{-2 \alpha X_i} \int_0^{X_i} e^{-(\alpha_0 - \alpha) y} e^{-\alpha_0 X_i} e^{-\alpha_0(X_i - y)}\left(  e^{-\alpha(X_i - y)}|D_i||d| + |c_i| + (e^{-\alpha_1 X_*} + |\lambda|)^2 |d| \right) dy \\
...
\end{align*}
where $s_0 > 0$ and $\gamma$ are the same as in Lemma \ref{IPform}. From the proof of Lemma \ref{jumpadj}, $\langle \Psi(X_j), Q'(-X_j) \rangle = - \langle \Psi(-X_j), Q'(X_j) \rangle$. Using equation \cref{Xjscale} for $X_j$, we have
\begin{align*}

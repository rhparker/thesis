...
WRITE THIS IN TERMS OF $m_j$ AND CHECK SIGN.
...
\tilde{a}_j(r) = s_0 e^{\alpha_0 \phi/\beta_0} b_j \left( \beta \cos\left(-\rho \log b_j \right) - \alpha \sin \left(-\rho \log b_j \right) \right) + \mathcal{O}(r^{\gamma/2\alpha_0})
\end{equation}
...
\subsection{Center eigenfunction}
...
If the eigenvalues of $\tilde{A}(0)$ are distinct, then the only solution to \cref{Lsystem1} when $\lambda = 0$ is $\partial_x Q(x)$.
\begin{proof}
Since the matrix $DF(0)$ is hyperbolic, solutions co \cref{Lsystem1} can be found using the method in \cite{Sandstede1998} adapted to periodic boundary conditions. Briefly, this works as follows. The setup is exactly the same as in \cite{Sandstede1998}, except periodic boundary conditions require one more matching condition at the tails of the pulses to ``close the loop''. Following the proof of \cite[Theorem 2]{Sandstede1998}, an eigenfunction $V(x)$ exists for sufficiently small $\lambda$ if and only if the $n$ jump conditions
\[
...
\[
|R(\lambda)d| \leq C \left( e^{-3 \alpha_1 X_*} + |\lambda|(e^{-\alpha_1 X_*} + |\lambda| ) \right)
\]
...
\end{equation*}
he jump conditions $\xi$ can be written in matrix form as in \cite[Theorem 2]{Sandstede1998} as
...
\]
By \cref{lemma:tildeA}, $A = r \tilde{A}(r)$, where $\tilde{A}(r)$ is smooth in $r$ and has an eigenvalue at 0 for all $r$. If the eigenvalues of $\tilde{A}(0)$ are distinct, $\tilde{A}(0)$ has a simple eigenvalue at 0. Following the proof of \cite[Theorem 3]{Sandstede1998}, equation \cref{Ljumps} has a simple zero at 0. Since this zero must correspond to $\partial_x Q(x)$, the result follows.
\end{proof}
\end{lemma}
...
If the eigenvalues of $\tilde{A}(0)$ are distinct, then there exists a function $V_n^c(x)$ which satisfies \cref{PDEeigsystemper3} for $\lambda = 0$.
\end{lemma}

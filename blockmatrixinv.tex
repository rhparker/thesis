...
V_a &= \bigoplus_{i=0}^{n-1} E^u(0) \oplus E^s(0) \oplus E^c(\lambda) \\
...
\begin{aligned}
Z&_i^-(x) = \Phi^s_{i,-}(x, -X_{i-1}; \lambda) P_-(-X_{i-1}, \lambda)^{-1} a_{i-1}^- + \Phi^u(x, 0; \lambda) b_i^- - \Phi^c(x, -X_{i-1}; \lambda) a_{i-1}^c V_0(\lambda) \\
...
Z&_i^+(x) = \Phi^u_{i,+}(x, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^+ + \Phi^s(x, 0; \lambda) b_i^+ + \Phi^c(x, X_i; \lambda) a_i^c V_0(\lambda) \\
...
\end{aligned}
\end{equation}
where $i = 0, \dots, n-1$. Since the center subspace is the span of the single vector $V_0(\lambda)$, $a_i^c \in \C$. Note that $a_i^+ \in E^u(0)$ and $a_{i-1}^- \in E^s(0)$, which are the eigenspaces corresponding to $\lambda = 0$. As in \cite{Sandstede1998}, we will solve the eigenvalue problem in a series of inversion steps. First, we will solve \cref{systemZ} for $Z_i^\pm$. To do this, we rewrite \eqref{Zfpeq} as
...
and
\begin{align*}
[L_2&(\lambda)_i^-(a,b,c,d)](x) = \Phi^s_{i,-}(x, -X_{i-1}; \lambda) P_-(X_{i-1}, \lambda) a_{i-1}^- + \Phi^u(x, 0; \lambda) b_i^- - \Phi^c(x, -X_{i-1}; \lambda)^{-1} a_{i-1}^c V_0(\lambda) \\
...
[L_2&(\lambda)_i^+(a,b,c,d)](x) = \Phi^u_{i,+}(x, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^+ + \Phi^s(x, 0; \lambda) b_i^+ + \Phi^c(x, X_i; \lambda) a_i^c V_0(\lambda) \\
...
\begin{align*}
Z_{i+1}^-&(-X_i) = \Phi^s_{i,-}(-X_i, -X_i; \lambda) P_-(-X_i, \lambda)^{-1} a_{i-1}^- + \Phi^u(-X_i, 0; \lambda) b_{i+1}^- - a_i^c V_0(\lambda) \\ 
...
Z_i^+&(X_i) = \Phi^u_{i,+}(X_i, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^+ + \Phi^s(X_i, 0; \lambda) b_i^+ + a_i^c V_0(\lambda) \\
...
\begin{aligned}
D_i &d + C_i c = P_-(-X_i, \lambda) \Phi^s_{i,-}(-X_i, -X_i; \lambda) P_-(-X_i, \lambda)^{-1} a_i^+ + (I + \Theta_+(X_i; \lambda))a_i^c V_0(\lambda) + P_+(X_i; \lambda)\Phi^s(X_i, 0; \lambda) b_i^+ \\
&+ P_+(X_i; \lambda) \int_0^{X_i} \Phi^s(X_i, y; \lambda) P_+(y; \lambda)^{-1}\big[ G_i^+(y) P_+(y; \lambda) Z_i^+(y) \\
&\qquad+ c_i e^{-\nu(\lambda)X_i} G_i^-(y) V^+(y; \lambda) + \lambda^2 d_i \tilde{H}_i^+(y)\big] dy \\
&- P_+(X_i, \lambda) \Phi^u_{i,+}(X_i, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^+ a_i^- + (I + \Theta_-(-X_i; \lambda))a_i^c V_0(\lambda) - P_-(-X_i; \lambda)\Phi^u(-X_i, 0; \lambda) b_{i+1}^- \\ 
...
\begin{align*}
P_-(-X_i, \lambda) \Phi^s_{i,-}(-X_i, -X_i; \lambda) P_-(-X_i, \lambda)^{-1} &= \tilde{\Phi}^s_{i,-}(-X_i, -X_i; \lambda) = P^u(0) \\
P_+(X_i, \lambda) \Phi^u_{i,+}(X_i, X_i; \lambda) P_+(X_i, \lambda)^{-1} &= \tilde{\Phi}^u_{i,+}(X_i, X_i; \lambda) = P^s(0),
\end{align*}
thus we have 

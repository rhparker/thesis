...
\begin{aligned}
Z&_i^-(x) = \Phi^s_{i,-}(x, -X_{i-1}; \lambda) P_-(-X_{i-1}, \lambda)^{-1} a_{i-1}^- + \Phi^u(x, 0; \lambda) b_i^- - \Phi^c_{i,-}(x, -X_{i-1}; \lambda) P_-(-X_{i-1}, \lambda)^{-1} a_{i-1}^c \\
...
&+ \int_{-X_{i-1}}^x \Phi^s_{i,-}(x, y; \lambda) P_-(y; \lambda)^{-1}[ G_i^-(y) P_-(y; \lambda)Z_i^-(y) + c_{i-1} e^{\nu(\lambda)X_{i-1}} G_i^-(y) V^-(y; \lambda) + \lambda^2 d_i \tilde{H}_i^-(y)] dy \\
&+ \int_{-X_{i-1}}^x \Phi^c_{i,-}(x, y; \lambda) P_-(y; \lambda)^{-1}[ G_i^-(y) P_-(y; \lambda)Z_i^-(y) + c_{i-1} e^{\nu(\lambda)X_{i-1}} G_i^-(y) V^-(y; \lambda) + \lambda^2 d_i \tilde{H}_i^-(y)] dy  \\ 
Z&_i^+(x) = \Phi^u_{i,+}(x, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^+ + \Phi^s(x, 0; \lambda) b_i^+ + \Phi^c_{i,+}(x, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^c \\
...
&+ \int_{X_i}^x \Phi^u_{i,+}(x, y; \lambda) P_+(y; \lambda)^{-1} [ G_i^+(y) P_+(y; \lambda) Z_i^+(y) + c_i e^{-\nu(\lambda)X_i} G_i^-(y)V^+(y; \lambda) + \lambda^2 d_i \tilde{H}_i^+(y)] dy \\
&+ \int_{X_i}^x \Phi^c_{i,+}(x, y; \lambda) P_+(y; \lambda)^{-1} [ G_i^+(y) P_+(y; \lambda) Z_i^+(y) + c_i e^{-\nu(\lambda)X_i} G_i^-(y)V^+(y; \lambda) + \lambda^2 d_i \tilde{H}_i^+(y)] dy \\
\end{aligned}
\end{equation}
As in \cite{Sandstede1998}, we will solve the eigenvalue problem in a series of inversion steps. First, we will solve \cref{systemZ} for $Z_i^\pm$. To do this, we rewrite \eqref{Zfpeq} as
...
&+ \int_{-X_{i-1}}^x \Phi^s_{i,-}(x, y; \lambda) P_-(y; \lambda)^{-1} G_i^-(y) P_-(y; \lambda) Z_i^-(y) dy \\
&+ \int_{-X_{i-1}}^x \Phi^c_{i,-}(x, y; \lambda) P_-(y; \lambda)^{-1} G_i^-(y) P_-(y; \lambda) Z_i^-(y) dy  \\ 
[L_1&(\lambda)_i^+ Z](x) = \int_0^x \Phi^s(x, y; \lambda) P_+(y; \lambda)^{-1} G_i^+(y) P_+(y; \lambda) Z_i^+(y) dy \\
&+ \int_{X_i}^x \Phi^u_{i,+}(x, y; \lambda) P_+(y; \lambda)^{-1} G_i^+(y) P_+(y; \lambda) Z_i^+(y) dy \\
&+ \int_{X_i}^x \Phi^c_{i,+}(x, y; \lambda) P_+(y; \lambda)^{-1} G_i^+(y) P_+(y; \lambda) Z_i^+(y) dy \\
\end{align*}
and
\begin{align*}
[L_2&(\lambda)_i^-(a,b,c,d)](x) = \Phi^s_{i,-}(x, -X_{i-1}; \lambda) P_-(X_{i-1}, \lambda) a_{i-1}^- + \Phi^u(x, 0; \lambda) b_i^- - \Phi^c_{i,-}(x, -X_{i-1}; \lambda)^{-1} P_-(X_{i-1}, \lambda) a_{i-1}^c \\
&+ \int_0^x \Phi^u(x, y; \lambda) P_-(y; \lambda)^{-1} [ c_{i-1} e^{\nu(\lambda)X_{i-1}} G_i^-(y)V^-(y; \lambda) + \lambda^2 d_i \tilde{H}_i^-(y)] dy \\
&+ \int_{-X_{i-1}}^x \Phi^s_{i,-}(x, y; \lambda) P_-(y; \lambda)^{-1} [c_{i-1} e^{\nu(\lambda)X_{i-1}}G_i^-(y)V^-(y; \lambda) + \lambda^2 d_i \tilde{H}_i^-(y)] dy \\
&+ \int_{-X_{i-1}}^x \Phi^c_{i,-}(x, y; \lambda) P_-(y; \lambda)^{-1}[c_{i-1} e^{\nu(\lambda)X_{i-1}} G_i^-(y)V^-(y; \lambda) + \lambda^2 d_i \tilde{H}_i^-(y)] dy \\
[L_2&(\lambda)_i^+(a,b,c,d)](x) = \Phi^u_{i,+}(x, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^+ + \Phi^s(x, 0; \lambda) b_i^+ + \Phi^c_{i,+}(x, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^c \\
&+ \int_0^x \Phi^s(x, y; \lambda) P_+(y; \lambda)^{-1} [c_i e^{-\nu(\lambda)X_i} G_i^+(y)V^+(y; \lambda) + \lambda^2 d_i \tilde{H}_i^+(y)] dy \\
&+ \int_{X_i}^x \Phi^u_{i,+}(x, y; \lambda) P_+(y; \lambda)^{-1} [c_i e^{-\nu(\lambda)X_i} G_i^+(y)V^+(y; \lambda) + \lambda^2 d_i \tilde{H}_i^+(y)] dy  \\
&+ \int_{X_i}^x \Phi^c_{i,+}(x, y; \lambda) P_+(y; \lambda)^{-1} [c_i e^{-\nu(\lambda)X_i} G_i^+(y)V^+(y; \lambda) + \lambda^2 d_i \tilde{H}_i^+(y)] dy 
\end{align*}
...
\begin{align*}
\Bigg| \int_{-X_{i-1}}^x &\Phi^c_{i,-}(x, y; \lambda) P_-(y; \lambda)^{-1} G_i^-(y) P_-(y; \lambda) Z_i^-(y) dy \Bigg| \\ 
...
\begin{align*}
\| \Phi^c_{i,-}(x, -X_{i-1}; \lambda) a_{i-1}^c \| \leq C |e^{-\nu(\lambda)X_{i-1}}|a_{i-1}^c| \leq C |a_{i-1}^c| \leq C |a|
\end{align*}
For the remaining terms which do not involve integrals,
\[
\| \Phi^s_{i,-}(x, -X_{i-1}; \lambda) P_-(-X_{i-1}, \lambda) a_{i-1}^- + \Phi^u(x, 0; \lambda) b_i^- \| \leq C(|a| + |b|)
\]
...
\begin{align*}
\Bigg| c_{i-1} &e^{\nu(\lambda)X_{i-1}} \int_{-X_{i-1}}^x \Phi^c_{i,-}(x, y; \lambda) P_-(y; \lambda)^{-1} G_i^-(y) V^-(y) dy \Bigg| \\ 
...
\Bigg| c_{i-1} e^{\nu(\lambda)X_{i-1}} \int_{-X_{i-1}}^x &\Phi^c_{i,-}(x, y; \lambda) P_-(y; \lambda)^{-1} G_i^-(y) V^-(y) dy \Bigg| \leq C e^{-\alpha X^*} |c| 
\end{align}
...
\begin{align*}
Z_{i+1}^-&(-X_i) = \Phi^s_{i,-}(-X_i, -X_i; \lambda) P_-(-X_i, \lambda)^{-1} a_{i-1}^-  - \Phi^c_{i,-}(-X_i, -X_i; \lambda) P_-(-X_i, \lambda)^{-1} a_i^c \\ 
&+ \Phi^u(-X_i, 0; \lambda) b_{i+1}^-
...
Z_i^+&(X_i) = \Phi^u_{i,+}(X_i, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^+ + \Phi^c_{i,+}(X_i, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^c \\
&+ + \Phi^s(X_i, 0; \lambda) b_i^+ 
...
\end{align*}
Applying the appropriate conjugation operators, subtracting, and using equation \eqref{projTheta}, we obtain the equation 
\begin{equation}\label{Dideq0}
\begin{aligned}
D_i &d + C_i c = P_+(X_i, \lambda) \Phi^u_{i,+}(X_i, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^+ +  P_+(X_i, \lambda) \Phi^c_{i,+}(X_i, X_i; \lambda) P_+(X_i, \lambda)^{-1} a_i^c \\
&+ P_+(X_i; \lambda)\Phi^s(X_i, 0; \lambda) b_i^+ + P_+(X_i; \lambda) \int_0^{X_i} \Phi^s(X_i, y; \lambda) P_+(y; \lambda)^{-1}\big[ G_i^+(y) P_+(y; \lambda) Z_i^+(y) \\
&\qquad+ c_i e^{-\nu(\lambda)X_i} G_i^-(y) V^+(y; \lambda) + \lambda^2 d_i \tilde{H}_i^+(y)\big] dy \\
&- P_-(-X_i, \lambda) \Phi^s_{i,-}(-X_i,-X_i; \lambda) P_-(-X_i, \lambda)^{-1} a_i^- + P_-(-X_i, \lambda) \Phi^c_{i,-}(-X_i,-X_i; \lambda) P_-(-X_i, \lambda)^{-1} a_i^c  \\ 
&-P_-(-X_i; \lambda)\Phi^u(-X_i, 0; \lambda) b_{i+1}^- - P_-(-X_i; \lambda) \int_0^{-X_i} \Phi^u(-X_i, y; \lambda) P_-(y; \lambda)^{-1}\big[ G_{i+1}^-(y) P_-(y; \lambda)Z_{i+1}^-(y) \\
&\qquad+ c_i e^{\nu(\lambda)X_i} G_{i+1}^-(y) V^-(y; \lambda) + \lambda^2 d_{i+1} \tilde{H}_{i+1}^-(y)\big] dy.
\end{aligned}
\end{equation}
From \cref{sec:trichotomy}, 
\begin{align*}
P_-(-X_i, \lambda) \Phi^{s/c}_{i,-}(-X_i, -X_i; \lambda) P_-(-X_i, \lambda)^{-1} &= \tilde{\Phi}^{s/c}_{i,-}(-X_i, -X_i; \lambda) = P^{s/c}(0) \\
P_+(X_i, \lambda) \Phi^{u/c}_{i,+}(X_i, X_i; \lambda) P_+(X_i, \lambda)^{-1} &= \tilde{\Phi}^{u/c}_{i,+}(X_i, X_i; \lambda) = P^{u/c}(0),
\end{align*}
thus we have
...
D_i d + C_i c &= a_i^+ - a_i^- + 2 a_i^c + L_3(\lambda)_i(a, b, c, d)
\end{align}
where the linear operator $L_3(\lambda)_i(a, b, c, d)$ is defined by the RHS of \cref{Didexpansion} (the dependence on $a$ is only via $Z_i^\pm$). $L_3(\lambda)_i(a, b, c, d)$ is linear in $(a,b,c,d)$ and analytic in $\lambda$. To get a bound on $L_3$, we will bound the individual terms. 
\begin{enumerate}
...
\begin{proof}
First, we apply the projections $P_0^{s/u}(0)$ on the eigenspaces $E^{s/u}(0)$ to \eqref{Dideq1}. Using the bound $|C_i c| \leq e^{-\alpha X^*}|c|$ from Lemma \ref{stabestimateslemma}, we obtain the expressions
\begin{align*}
...
\begin{proof}
First, we look at the stable projection $\tilde{P}^s_+(X_i; \lambda) = P_+(X_i; \lambda) \Phi^s_+(X_i, X_i; \lambda) P_+^{-1}(X_i; \lambda)$. Since the range of the complement projection is not uniquely determined, we are free to choose any complement of $\ran \tilde{P}^s_+(X_i; \lambda)$. In particular, since for sufficiently small $\lambda$, $\R^{2m+1} = E^s(\lambda) \oplus E^u(0) \oplus E^c(0)$, we can choose the range of the complement projection to be $P_+(X_i; \lambda)$

...
\begin{proof}
We want to solve the center jump condition, which are given by
\[
\xi_i^c = 
...
&= \lambda^2 d_i \int_{-X_{i-1}}^0 \langle \tilde{\Phi}^-(y, -X_{i-1}; 0)^* P^c(0)^* \tilde{\Phi}_-(-X_{i-1}, 0; 0)^* W_0,
...
&\qquad\leq C |\lambda| e^{-\alpha X^*} \left( |c| + (|\lambda| + e^{-\alpha X^*})^2  |d| \right)
\end{align*}
The term involving $a_{i-1}^c$ is similar.
...
\begin{align*}
\left| (P^i_+(0; \lambda) - P^i_+(0; 0))b_i^+ + P^i_+(0; \lambda) (P^s(\lambda) - P^s(0)) b_i^+ \right| \leq C |\lambda| \left( e^{-\alpha X^*}|c| + (|\lambda| + e^{-\alpha X^*})^2 |d| \right)
\end{align*}
The $b_i^-$ term is similar.

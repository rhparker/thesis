...
\end{align*}
We have similar expressions for $P^s(0) Q'(X_i)$ and $P^u(0) Q'(X_i)$. Substituting these into \cref{aipm1}, we have 
\begin{equation}\label{aipm2}
\begin{aligned}
a_i^+ &= P^u(0) Q'(X_i) d + P^u(0) \left( L_3(\lambda)_i(a, b, c, d) + C_i c + \mathcal{O}( e^{-\alpha X_i} (e^{-\alpha X^*} + |\lambda| )|d|) \right) \\
a_i^- &= -P^s(0) Q'(-X_i) d + P^s(0) \left( L_3(\lambda)_i(a, b, c, d) + C_i c + \mathcal{O}( e^{-\alpha X_i} (e^{-\alpha X^*} + |\lambda| )|d|) \right)
\end{aligned}
\end{equation}
Using the bound $C_i c = \mathcal{O}(e^{-\alpha X^*}|c|)$, substituting $A_1$ from \cref{Zinv1} for $a$ in $L_3(\lambda)_i(a, b, c, d)$, and simplifying, we obtain the expressions \cref{aipmexp1}, where $A_2$ comprises the remainder terms in \cref{aipm2} and has bound
\begin{align*}

...
a_i^c &= -2 \lambda q(X_i) (d_{i+1} - d_i ) + \tilde{A}_2(\lambda)_i(b, c, d)
\end{align}
...
a_i^c &= -2 \lambda q(X_i)(d_{i+1} - d_i ) + \tilde{A}_4(\lambda)_i(c, d) 
\end{align*}
...
&- 2 \lambda q(X_i) (d_{i+1} - d_i ) - \lambda^2 \tilde{M}^c d_i + R^c(\lambda)_i(c, d)
\end{aligned}
\end{equation}
where
\begin{align*}
...
\left( K(\lambda) -\frac{1}{2} \lambda \tilde{M} K_1(\lambda) + C_1 \right) c + \left(- 2 \lambda A_1 - \lambda^2 \tilde{M}^c I + D_1\right) d = 0.
\end{equation}
...
&= e^{-\nu(\lambda)X_i} 2 \lambda q(X_i) (d_{i+1} - d_i ) + \mathcal{O}\left( |\lambda| (|\lambda| + e^{-\alpha X^*})|c| + |\lambda| e^{-\alpha X^*}(|\lambda| + e^{-\alpha X^*}) |d|) \right)
\end{align*}
...
= -2 \lambda q(X_i) (d_{i+1} - d_i ) + \mathcal{O}\left( |\lambda| (|\lambda| + e^{-\alpha X^*})|c| + |\lambda| e^{-\alpha X^*}(|\lambda| + e^{-\alpha X^*}) |d|) \right)
\end{align*}
...
For the leading order terms involving $d$, the term involving $\tilde{M}^c$ is $-\lambda^2 \tilde{M}^c I$. The other terms involving $d$ are given by $-2 \lambda A_1 d$, where $d = (d_1, \dots, d_{n-1}, d_0)^T$ and 
\begin{align*}
A_1 &= \begin{pmatrix}
-q(X_1) & q(X_1) &&& \\
 & -q(X_2) & q(X_2) \\

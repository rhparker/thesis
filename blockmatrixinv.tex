...
V_g &= \bigoplus_{i=0}^{n-1} E^c(\lambda) = \bigoplus_{i=0}^{n-1} \C V_0(\lambda) \\
...
\begin{aligned}
Z&_i^-(x) = \Phi^s(x, -X_{i-1}; \lambda) a_{i-1}^- + \Phi^u(x, 0; \lambda) b_i^- - \Phi^c(x, -X_{i-1}; \lambda) g_{i-1} \\
&+ \int_0^x \Phi^u(x, y; \lambda)[P_-(y; \lambda)^{-1} G_i^-(y) P_-(y; \lambda)Z_i^-(y) + c_i P_-(x; \lambda)^{-1} G_i^-(x)V^-(x; \lambda) + \lambda^2 d_i P_-(y; \lambda)^{-1} \tilde{H}_i^-(y)] dy \\
&+ \int_{-X_{i-1}}^x \Phi^s(x, y; \lambda)[P_-(y; \lambda)^{-1} G_i^-(y) P_-(y; \lambda)Z_i^-(y) + c_i P_-(x; \lambda)^{-1} G_i^-(x)V^-(x; \lambda) + \lambda^2 d_i P_-(y; \lambda)^{-1} \tilde{H}_i^-(y)] dy \\
&+ \int_{-X_{i-1}}^x \Phi^c(x, y; \lambda) [P_-(y; \lambda)^{-1} G_i^-(y) P_-(y; \lambda)Z_i^-(y) + c_i P_-(x; \lambda)^{-1} G_i^-(x)V^-(x; \lambda) + \lambda^2 d_i P_-(y; \lambda)^{-1} \tilde{H}_i^-(y)] dy  \\ 
Z&_i^+(x) = \Phi^u(x, X_i; \lambda) a_i^+ + \Phi^s(x, 0; \lambda) b_i^+ + \Phi^c(x, X_i; \lambda) g_i \\
&+ \int_0^x \Phi^s(x, y; \lambda) [P_+(y; \lambda)^{-1} G_i^+(y) P_+(y; \lambda) Z_i^+(y) + c_i P_+(x; \lambda)^{-1} G_i^+(x)V^+(x; \lambda) + \lambda^2 d_i P_+(y; \lambda)^{-1} \tilde{H}_i^+(y)] dy \\
&+ \int_{X_i}^x \Phi^u(x, y; \lambda) [P_+(y; \lambda)^{-1} G_i^+(y) P_+(y; \lambda) Z_i^+(y) + c_i P_+(x; \lambda)^{-1} G_i^+(x)V^+(x; \lambda) + \lambda^2 d_i P_+(y; \lambda)^{-1} \tilde{H}_i^+(y)] dy \\
&+ \int_{X_i}^x \Phi^c(x, y; \lambda) [P_+(y; \lambda)^{-1} G_i^+(y) P_+(y; \lambda) Z_i^+(y) + c_i P_+(x; \lambda)^{-1} G_i^+(x)V^+(x; \lambda) + \lambda^2 d_i P_+(y; \lambda)^{-1} \tilde{H}_i^+(y)] dy \\
\end{aligned}
\end{equation}
where $i = 0, \dots, n-1$. As in \cite{Sandstede1998}, we will solve the eigenvalue problem in a series of inversion steps. First, we will solve equation \eqref{systemZ} for $Z_i^\pm$. To do this, we rewrite \eqref{Zfpeq} as
...
and
\begin{align*}
[L_2&(\lambda)_i^-(a,b,c,\tilde{c},d)](x) = \Phi^s(x, -X_{i-1}; \lambda) a_{i-1}^- + \Phi^u(x, 0; \lambda) b_i^- - \Phi^c(x, -X_{i-1}; \lambda) g_{i-1} \\
...
[L_2&(\lambda)_i^+(a,b,c,\tilde{c},d)](x) = \Phi^u(x, X_i; \lambda) a_i^+ + \Phi^s(x, 0; \lambda) b_i^+ + \Phi^c(x, X_i; \lambda) g_i \\
...
\| Z_1(\lambda)(a,b,c,\tilde{c},d) \| \leq C\left(|a| + |b| + |g| + |c| + e^{-\alpha X_*}|\lambda|^2|d|\right)
\end{equation}
\begin{proof}
...
\begin{aligned}
D_i &d + C_i c = (I + \Theta_+(X_i; \lambda))a_i^+ + (I + \Theta_+(X_i; \lambda))g_i + P_+(X_i; \lambda)\Phi^s(X_i, 0; \lambda) b_i^+ \\
...
&- (I + \Theta_-(-X_i; \lambda))a_i^- + (I + \Theta_-(-X_i; \lambda))g_i - P_-(-X_i; \lambda)\Phi^u(-X_i, 0; \lambda) b_{i+1}^- \\ 
...
\begin{aligned}
D_i &d = a_i^+ - a_i^- + 2 g_i \\
&+ \Theta_+(X_i; \lambda)a_i^+ - \Theta_-(-X_i; \lambda)a_i^- + \Theta_+(X_i; \lambda)g_i + \Theta_-(-X_i; \lambda)g_i\\
...
D_i d + C_i c &= a_i^+ - a_i^- + 2 g_i + L_3(\lambda)_i(a, b, c, \tilde{c}, d)
\end{align}
where the linear operator $L_3(\lambda)_i(a, b, c, \tilde{c}, d)$ is defined by the RHS of \cref{Didexpansion}. $L_3(\lambda)_i(a, b, g, c, d)$ is linear in $a,b,g,c$ and $d$ and analytic in $\lambda$. To get a bound on $L_3$, we will bound the individual terms. 
\begin{enumerate}
...
\]
\item For the terms involving $g_i$, we use \eqref{conjthetadecay} to get
\[
|\Theta_+(X_i; \lambda)g_i + \Theta_-(-X_i; \lambda)g_i| \leq 
C e^{-\alpha X_*} |g|
\]
...
&\leq C \left(|a| + |b| + |g| + |c| + e^{-\alpha X_*}|\lambda|^2|d|\right) \int_0^{X_i} e^{-\tilde{\alpha}(X_i - y)} \left(e^{-\alpha(X_i - y)}e^{-\alpha X_i} + e^{-2 \alpha X_{i-1}} e^{-\alpha y} \right) dy \\
&\leq C e^{-\alpha X_*} \left(|a| + |b| + |g| + |c| + e^{-\alpha X_*}|\lambda|^2|d|\right)
\end{align*}
...
|L_3(\lambda)(a, b, g, c, d)| \leq C \left( e^{-\alpha X_*} (|a| + |g| + |\lambda|^2 |d|) + e^{-\tilde{\alpha} X_*} |b| + |c| \right)
\end{equation}
As in \cref{Zinv0}, $e^{-\alpha X_*} < \delta$, thus \cref{L3bound} becomes
\begin{equation*}
|L_3(\lambda)(a, b, g, c, d)| \leq C \left( \delta (|a| + |g|) + e^{-\alpha X_*}|\lambda|^2 |d| + e^{-\tilde{\alpha} X_*} |b| + |c| \right)
\end{equation*}
Let 
\[
...
\]
be defined by $(J_1)_i(a_i^+, a_i^-, g_i) = a_i^+ - a_i^- + 2 g_i$. The map $J_i$ is a linear isomorphism since 
\[
E^s(\lambda) \oplus E^u(\lambda) \oplus E^c(\lambda) = \C^{2m+1}.
\]
Consider the map
\[
S_1(a, \tilde{c}) = J_1 (a, g) + L_3(\lambda)(a, 0, c, 0, 0) = J_1( I + J_1^{-1} L_3(\lambda)(a, 0, c, 0, 0))
\]
Decreasing $\delta$ if necessary, $\|J_1^{-1} L_3(\lambda)(a, 0, \tilde{c}, 0, 0)\| < 1$, thus the operator $S_1(a, g)$ is invertible. We can then solve for $(a, g)$ to get
\[
(a, g) = A_1(\lambda)(b, c, d) = S_i^{-1}(-D d - C_i c + L_3(\lambda)(0, b, 0, c, d)
\]
...
In the next two lemmas, we derive expressions for $a_i^\pm$ and $g_i$ which we will use in evaluating the jump conditions. First, we have the following expressions for $a_i^\pm$.
...
Next, we derive an expression for $g_i$. Before we do this, we will evaluate an inner product term which will appear in that expression.

...
\begin{proof}
Apply the projection $P^c(\lambda)$ on $E^c(\lambda)$ to the matching condition $P_i^+(X_i; \lambda) Z_i^+(X_i) - P_{i+1}^-(-X_i; \lambda) Z_{i+1}^-(-X_i) = D_i d + C_i c$ to get
...
Next, we evaluate $P_c(\lambda) C_i c$ and $P^c(\lambda)D_i d$. From Lemma \ref{stabestimateslemma}, $|C_i c| \leq C e^{-\alpha X^*}$. 

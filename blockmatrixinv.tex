...
\begin{align*}
\Bigg| c_i \int_{-X_{i-1}}^x &\Phi^c(x, y; \lambda) P_-(y; \lambda)^{-1} G_i^-(y) V^-(y) dy \Bigg| \\ 
&\leq C |c| \int_{-X_{i-1}}^x \left| e^{\nu(\lambda)(x - y)} \right| \left|e^{\nu(\lambda)(y)}\right| \left( e^{-\alpha X_{i-1}} e^{-\alpha(X_{i-1} + y) } + e^{-2 \alpha X_i} e^{\alpha y} \right) dy
\end{align*}
Using \cref{lemma:expnubound}, $\left| e^{\nu(\lambda)(x - y)} \right| \left|e^{\nu(\lambda)(y)}\right| \leq 2 R_N$, which is a constant depending on $N$. Following the same procedure as in Lemma \ref{L1boundlemma}, we thus obtain the bound
\begin{align}\label{intGVpmbound}
\Bigg| c_i \int_{-X_{i-1}}^x &\Phi^c(x, y; \lambda) P_-(y; \lambda)^{-1} G_i^-(y) V^-(y) dy \Bigg| \leq C e^{-\alpha X_*} |c| 
\end{align}
...
\item The integral terms involving $c_i G_i^\pm(x) V^\pm(x; \lambda)$ are bounded by $C e^{-\alpha X_*} |c|$ as in \cref{intGVpmbound}. 
\end{enumerate}
...
\]
Using the bounds on $L_3$, $A_1$ has bound given by \cref{A1bound}, where we bound $C_i$ by a constant due to \cref{lemma:expnubound}. We then plug this into $Z_1$ from Lemma \ref{Zinv0} to get the operator $Z_2(\lambda)(b,c,d)$ which has bound given by \cref{Z1bound}.
\end{proof}
\end{lemma}
...
\end{equation}
When we apply the projections $P_0^{s/u}(\lambda)$, we obtain the expressions
\begin{align*}
...
Next, we evaluate $P^c(\lambda)C_i c$ and $P^c(\lambda)D_i d$. Using \cref{Vpmestimate}, we have
...
We will now put this all together to obtain the center jump expressions. Since we have remainder terms of order $\mathcal{O}(e^{-\alpha X_*} + |\lambda|)|c|$, the term \cref{centerVjump} will also be a remainder term since it is of the same order. The center jump expressions are given by
\begin{align*}
\xi^c_i &= \frac{1}{2} \left( c_{i+1} e^{-2 \nu(\lambda)X_i} - c_{i-1} e^{2 \nu(\lambda)X_{i-1}} \right) + e^{-\nu(\lambda)X_i} \lambda q(X_i) (d_{i+1} - d_i )  + e^{\nu(\lambda)X_{i-1}} \lambda q(X_{i-1}) (d_i - d_{i-1} ) \\
&\qquad - \lambda^2 \tilde{M}^c d_i + R^c(\lambda)_i(c, d)
\end{align*}
...
\left(-\lambda M^c K_1(\lambda) + C_2 \right)c + (A - \lambda^2 M I + D_2)d = 0.
\end{equation}
...
\end{align}
The terms $P_\pm(0; \lambda) Z_i^\pm(0)$ are given by
\begin{align*}
...
&= -\frac{1}{2} M^c \lambda \left( \frac{1}{2}e^{-\nu(\lambda)X_i} \left( c_{i+1} e^{-\nu(\lambda)X_i} - c_i e^{\nu(\lambda)X_i} \right)\right) \\
&\qquad + \mathcal{O}\left( |\lambda|(|\lambda| + e^{-\alpha X_*})|c| + |\lambda| (|\lambda| + e^{-\alpha X_*})^2 |d|) \right)\\
&= -\frac{1}{4} M^c \lambda \left( c_{i+1} e^{-2 \nu(\lambda)X_i} - c_i \right) + \mathcal{O}\left( |\lambda|(|\lambda| + e^{-\alpha X_*})|c| + |\lambda| (|\lambda| + e^{-\alpha X_*})^2 |d|) \right)
\end{align*}
where for the remainder terms we used $\langle \Psi(0), V^\pm(0, \lambda) \rangle = \mathcal{O}(|\lambda|)$. Similarly, again using \cref{lemma:VpmPsiIP},
\begin{align*}
\langle \Phi(0), &P_-(0; \lambda) e^{\nu(\lambda) X_{i-1}} a_{i-1}^c \rangle = e^{\nu(\lambda)X_i} a_{i-1}^c \langle \Psi(0), V^-(0; \lambda) \rangle \\
&= \frac{1}{4} M^c \lambda \left( c_i - c_{i-1} e^{2 \nu(\lambda)X_i} \right) + \mathcal{O}\left( |\lambda|(|\lambda| + e^{-\alpha X_*})|c| + |\lambda| (|\lambda| + e^{-\alpha X_*})^2 |d|) \right)
\end{align*}
Adding these together,
\begin{align}\label{Psiaic}
...
Putting this all together and noting that the $c_i$ term changes signs when we add \cref{PsiVpmjump} and \cref{Psiaic}, we have the jump expressions
\begin{align*}
\xi_i = \langle \Psi&(X_i), Q'(-X_i) \rangle (d_{i+1} - d_i ) + \langle \Psi(-X_{i-1}), Q'(X_{i-1}) \rangle (d_i - d_{i-1} ) \\
&-\frac{1}{4} M^c \lambda \left(c_{i+1} e^{-2 \nu(\lambda)X_i} + 2 c_i + c_{i-1} e^{2 \nu(\lambda)X_i} \right)
- \lambda^2 d_i M + R_i(\lambda)(c, d)
...
As in Lemma \ref{jumpcenteradj}, we will write these jump expressions in matrix form. The leading order terms involving $c$ are given by the matrix $-\lambda M^c K_1(\lambda)c$, which is the periodic tri-diagonal matrix
\begin{align*}
K_1(\lambda) = \frac{1}{4}
\begin{pmatrix}
2 & e^{-2\nu(\lambda)X_1} & & & & e^{2 \nu(\lambda)X_0} \\

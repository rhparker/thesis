...
\end{aligned}
\end{equation}
where $i = 0, \dots, n-1$. Since the center subspace is the span of the single vector $V_0(\lambda)$, $a_i^c \in \C$. Note that $a_i^+ \in E^u(0)$ and $a_i^- \in E^s(0)$, which are the eigenspaces corresponding to $\lambda = 0$. As in \cite{Sandstede1998}, we will solve the eigenvalue problem in a series of inversion steps. First, we will solve \cref{systemZ} for $Z_i^\pm$. To do this, we rewrite \eqref{Zfpeq} as
...
\begin{align*}
Z_{i+1}^-&(-X_i) = \Phi^s(-X_i, -X_i; \lambda) a_{i-1}^- + \Phi^u(-X_i, 0; \lambda) b_{i+1}^- - a_i^c V_0(\lambda) \\ 
...
Z_i^+&(X_i) = \Phi^u(X_i, X_i; \lambda) a_i^+ + \Phi^s(X_i, 0; \lambda) b_i^+ + a_i^c V_0(\lambda) \\

...
\begin{proof}
We apply the projections $P_0^{s/u}(0)$ on the eigenspaces $E^{s/u}(0)$ to \eqref{Dideq1}. Using the bound $|C_i c| \leq e^{-\alpha X^*}|c|$ from Lemma \ref{stabestimateslemma}, we obtain the expressions
\begin{align*}
a_i^+ &= P_0^u(\lambda) D_i d + P_0^u L_3(\lambda)_i(a, b, c, d) + \mathcal{O}(e^{-\alpha X^*}|c|) \\
a_i^- &= -P_0^s(\lambda) D_i d + P_0^s L_3(\lambda)_i(a, b, c, d) + \mathcal{O}(e^{-\alpha X^*}|c|).
\end{align*}
Let 
\begin{align*}
A_2(\lambda)_i^+(b, c, d) = P_0^u L_3(\lambda)_i(a, b, c, d) + \mathcal{O}(e^{-\alpha X^*}|c|)\\
A_2(\lambda)_i^-(b, c, d) = P_0^s L_3(\lambda)_i(a, b, c, d) + \mathcal{O}(e^{-\alpha X^*}|c|),
\end{align*}
where we substitute $A_1$ from \cref{Zinv1} for $a$ in $L_3(\lambda)_i(a, b, c, d)$. Using the bound for $A_1$ together with the bound for $L_3$ and simplifying, we obtain the remainder bound
\begin{align*}
...
\end{align*}
since $R$ is self-adjoint, $RW_0 = W_0$, and $RV_0 = V_0$. For the terms involving $a_i^c$, $\langle W_0, a_i^c \rangle = P^c(0) a_i^c = a_i^c$, since $a_i^c \in E^c(0)$. Finally, we evaluate $\langle W_0, L_3(\lambda)_i(a,b,c,d)$. 

Next, we look at the term involving $a_i^c$. First, we note that $P^c(\lambda) V_0 = (I + \mathcal{O}(|\lambda|)) V_0$ and $P^c(\lambda) V_0 \in V_0(\lambda)$, thus we can write $P^c(\lambda) V_0 = V_0 + k \lambda V_0'(0) + \mathcal{O}(|\lambda|^2)$ for some constant $k$. Using this and expanding the conjugation operators, we have
\begin{align*}
\langle W_0, &(P_+(X_i; \lambda) +  P_-(-X_i; \lambda)) P^c(\lambda) a_i^c V_0 \rangle 
= a_i^c \langle W_0, (P_+(X_i; \lambda) +  P_-(-X_i; \lambda))(V_0 + k \lambda V_0'(0) + \mathcal{O}(|\lambda|^2)) \rangle \\
&= a_i^c \langle W_0, (I + \Theta_+(X_i; \lambda) + I + R\Theta_+(X_i; -\lambda)R)(V_0 + k \lambda V_0'(0) + \mathcal{O}(|\lambda|^2)) \rangle \\
&= a_i^c \langle W_0, (I + \Theta_+(X_i; 0) + \lambda \partial_\lambda \Theta_+(X_i; 0) + I + R\Theta_+(X_i; 0)R - \lambda R \partial_\lambda \Theta_+(X_i; 0)R + \mathcal{O}(|\lambda|^2))\\
&\qquad(V_0 + k \lambda V_0'(0) + \mathcal{O}(|\lambda|^2)) \rangle \\
&= a_i^c \langle W_0, (P_+(X_i; 0) + P_-(X_i; 0) + \lambda (\partial_\lambda \Theta_+(X_i; 0) - R \partial_\lambda \Theta_+(X_i; 0)R) + \mathcal{O}(|\lambda|^2))
(V_0 + k \lambda V_0'(0) + \mathcal{O}(|\lambda|^2)) \rangle \\
&= a_i^c \Big( \langle W_0, (P_+(X_i; 0) + P_-(X_i; 0) \rangle \Big)
\end{align*}
\begin{align*}
\langle W_0, &(P_+(X_i; \lambda) +  P_-(-X_i; \lambda)) P^c(\lambda) a_i^c V_0 \rangle 
= a_i^c \langle W_0, (P_+(X_i; \lambda) +  P_-(-X_i; \lambda))(V_0 + k \lambda V_0'(0) + \mathcal{O}(|\lambda|^2)) \rangle \\
&= a_i^c \langle W_0, (I + \Theta_+(X_i; \lambda) + I + R\Theta_+(X_i; -\lambda)R)(V_0 + k \lambda V_0'(0) + \mathcal{O}(|\lambda|^2)) \rangle \\
&= a_i^c \langle W_0, (I + \Theta_+(X_i; 0) + I + R\Theta_+(X_i; 0)R + \mathcal{O}(e^{-\alpha X^*} |\lambda|))(V_0 + k \lambda V_0'(0) + \mathcal{O}(|\lambda|^2)) \rangle \\
&= a_i^c \Big( \langle W_0, (P_+(X_i; 0) + P_-(X_i; 0)) V_0 \rangle + k \lambda \langle W_0, (P_+(X_i; 0) + R P_+(X_i; 0) R ) V_0'(0)\rangle \Big) + \mathcal{O}(|\lambda|( e^{-\alpha X^*} + |\lambda|)|a|)
\end{align*}
...
\begin{align*}
\langle W_0, &(P_+(X_i; 0) + R P_+(X_i; 0) R ) V_0'(0)\rangle = 
\langle W_0, P_+(X_i; 0) V_0'(0) \rangle - \langle W_0, P_+(X_i; 0) V_0'(0) \rangle = 0,
\end{align*}
since $R V_0'(0) = -V_0'(0)$. Thus we are left with 

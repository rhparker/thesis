...
In this section, we will look at primary pulse solutions to \eqref{eqODE}, which are bound states of \eqref{genPDE}. These are solutions to \eqref{eqODE} which are exponentially localized, and can be thought of as traveling pulses. We will use a spatial dynamics approach and view a primary pulse solution as a homoclinic orbit connecting a baseline state to itself. 

To do this, we will first write equation \eqref{eqODE} as a first order system in the standard way, which we can do by  Hypothesis \ref{Ehyp}(iv) since the highest order derivative $\partial_x^{2m}$ in $E'(u)$ appears by itself with a coefficient of 1. Since we are interested in bound states, we will take $c$ from \eqref{eqODE} as a parameter. Thus we obtain the equivalent first-order system
\begin{equation}\label{genODE}
U'(x) = F(U(x); c)
\end{equation}
where $U = (u, \partial_x u, \dots, \partial_x^{2m-1} u) \in \R^{2m}$, $F: \R^{2m} \times \R \rightarrow \R^{2m}$ is smooth, and $F(0; c) = 0$ for all $c$. Reversibility implies that
...
For the same $c_0$ as in Hypothesis \ref{hypeqhyp}, a homoclinic orbit solution $Q(x; c_0) \in W^s(0) \cap W^u(0) \subset H^{-1}(0)$ exists to \eqref{genODE}. Furthermore, $\nabla H(Q(0; c_0)) \neq 0$ and $Q(x; c_0)$ is symmetric with respect to the reversor operator \eqref{reverserR2m}, i.e. $Q(-x; c_0) = R Q(x; c_0)$.
\end{hypothesis}
...
In this section, we will prove the existence of periodic multi-pulse solutions to \eqref{genODE}. The existence of multi-pulse solution on $\R$ follows from \cite{SandstedeStrut}.

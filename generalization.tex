...
\item $\nabla H(u; c) = 0$ if and only if $F(u; c) = 0$
\item For all $u \in \R^{2m}$ and all $c$,
\begin{equation}
\langle \nabla H(u; c), F(u; c) \rangle = 0
\end{equation}
\end{enumerate}
\end{hypothesis}
...
For a specific $c_0 \in \R$, $U = 0$ is a hyperbolic equilibrium of \eqref{genODE}, i.e. $DF(0; c_0)$ has no eigenvalues on the imaginary axis. Furthermore, the spectrum of $DF(0; c_0)$ contains a quartet of simple eigenvalues $\pm \alpha_0 \pm \beta_0 i$, $\alpha_0, \beta_0 > 0$, and for any other eigenvalue $\nu$ of $DF(0; c_0)$, $|\text{Re }\nu| > \alpha_0$.
\end{hypothesis}
Let $W^s(0; c_0)$ and $W^u(0; c_0)$ be the stable and unstable manifolds of the equilibrium at 0, and let $E_0^s(c_0)$ and $E_0^u(c_0)$ be the stable and unstable eigenspaces of $DF(0; c_0)$. By reversibility, all of these spaces have dimension $m$. 
...
For the same $c_0$ as in Hypothesis \ref{hypeqhyp}, there exists a homoclinic orbit solution $Q(x; c_0) \in W^s(0) \cap W^u(0) \subset H^{-1}(0)$ to \eqref{genODE}. Furthermore, $Q(0; c_0) \neq 0)$, $\nabla H(Q(0; c_0)) \neq 0$, and $Q(x; c_0)$ is symmetric with respect to the reversor operator \eqref{reverserR2m}, i.e. $Q(-x; c_0) = R Q(x; c_0)$.
\end{hypothesis}
...
The zero level set of $H^{-1}(0; c_0)$ contains a $(m-1)$-dimensional manifold in a neighborhood of $Q(0; c_0)$.
\end{lemma}
...
Using this hypothesis, we can prove the existence of homoclinic orbits $Q(x; c)$ for $c$ near $c_0$ and show that $Q(x; c)$ is differentiable in $c$.

\begin{lemma}\label{transverseint}
Assume Hypothesis \ref{H0transversehyp}. Then there exists $\delta > 0$ such that for $c \in (c_0 - \delta, c_0 + \delta)$, there is a unique transvese intersection of the stable and unstable manifolds $W^s(0; c)$ and $W^u(0; c)$. Let $Q(x; c)$ be the homoclinic orbit associated with this unique intersection. Then there is a $C^1$ map $c \rightarrow Q(x; c)$, i.e. $Q(x; c)$ is differentiable with respect to $c$ for $c \in (c_0 - \delta, c_0 + \delta)$. Furthermore, $\partial_c Q(x; c)$ is exponentially localized.
\end{lemma}

This lemma implies that Hypothesis \ref{cintervalhyp} is satisfied. From this point forward, we will only consider the case when $c = c_0$, thus we will omit the dependence on $c$ for simlicity of notation. We will also use $c$ in place of $c_0$.

Since $Q(x) \subset W^s(0) \cap W^u(0)$, 
\[
Q'(x) \in T_{Q(x)}W^s(0) \cap T_{Q(x)}W^u(0)
\]
In the next hypothesis, we will assume that this intersection to be one-dimensional, which is a standard nondegeneracy condition.

\begin{hypothesis}\label{nondegenhyp}
We take the non-degeneracy condition
\begin{equation}
...
Finally, we will decompose the tangent spaces of the stable and unstable manifolds at $Q(0)$. By Hypothesis \ref{nondegenhyp}, the two tangent spaces have a one-dimensional intersection spanned by $Q'(0)$, so we can write
\begin{align*}
...
Assume Hypotheses \ref{Ehyp}, \ref{Hhyp}, \ref{hypeqhyp}, \ref{Qexistshyp}, and \ref{nondegenhyp}. Let $Q(x)$ be a transversely constructed, symmetric primary pulse solution to \eqref{genODE}. Choose
\begin{itemize}
...
Assume Hypotheses \ref{Ehyp}, \ref{Hhyp}, \ref{hypeqhyp}, \ref{Qexistshyp}, and \ref{nondegenhyp}. Let $Q(x)$ be a transversely constructed, symmetric primary pulse solution to \eqref{genODE}. For every $M \in \N$ there exists $r_0 > 0$ such that for every $r \in \mathcal{R}$ with $r < r_0$, the following is true.
\begin{enumerate}[(i)]
...
T_{Q(0)}W^u(0) &= \R Q'(0) \oplus Y^- \\
\end{align*}
Since $\dim \R Q'(0) \oplus Y^+ \oplus Y^- = 2m-1$, there are two remaining directions needed in order to span $\R^{2m+1}$. To find these, let $V_0$ and $W_0$ be eigenvectors of $A(0)$ and $-A^*(0)$ corresponding to the simple eigenvalue 0. From the proof of the next lemma, we can scale $V_0$ so that $\langle V_0, W_0 \rangle = 1$. We have the following two lemmas regarding solutions to \eqref{vareq2} and \eqref{adjvareq2}.
...
Assume Hypotheses \ref{Ehyp}, \ref{Hhyp}, \ref{hypeqhyp}, \ref{Qexistshyp}, and \ref{nondegenhyp}. Let $q_{np}(x)$ be a periodic $n-$pulse solution constructed according to Theorem \ref{perexist} with lengths $X_0, \dots, X_{n-1}$. Then there exists $\delta \leq \delta_0$ with the following property: there exists a bounded, nonzero solution $V$ of \eqref{PDEeig} for $|\lambda| < \delta$ if and only if the block matrix equation

...
\Phi^c_{i,+}(x, y; \lambda) &= \Phi(x, X_i; \lambda) P_+(X_i, \lambda)^{-1}
P^c(0) P_+(X_i, \lambda) \Phi(X_i, x; \lambda) && 0 \leq y \leq x \\
\Phi^s_{i,-}(x, y; \lambda) &= \Phi(x, -X_{i-1}; \lambda) P_-(-X_{i-1}, \lambda)^{-1}
P^s(0) P_-(-X_{i-1}, \lambda) \Phi(-X_{i-1}, x; \lambda) && y \leq x \leq 0 \\
\Phi^c_{i,-}(x, y; \lambda) &= \Phi(x, -X_{i-1}; \lambda) P_-(-X_{i-1}, \lambda)^{-1}
P^c(0) P_-(-X_{i-1}, \lambda) \Phi(-X_{i-1}, x; \lambda) && y \leq x \leq 0 \\
\end{aligned}
\end{equation}
Since $|\lambda| < \delta$ and the $X_i$ are fixed, estimates for these are the same as for the corresponding evolutions in \cref{Zevolbounds}, although the constant $C$ may be larger.
...
\end{aligned}
\end{equation}
For $i = 0, \dots, n-1$, the exponential trichotomies for \cref{constcoeffZ} induce exponential trichotomies for the unconjugated system on $\R^\pm$ via the equations
...
&= P^u(0),
\end{align*}
which the unstable eigenprojection for $A(0)$ and is independent of $\lambda$. Similarly, for the center projection on $\R^+$, we have
\begin{align*}

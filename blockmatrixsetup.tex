...
\end{aligned}
\end{equation}
Both of these kernel eigenfunctions satisfy the periodic boundary conditions. We will use Lin's method to construct solutions $V(x)$ to \cref{PDEeigsystemper3} as piecewise perturbations of the kernel eigenfunctions $\partial_x Q_n(x)$ and $\partial_c Q_n(x)$. As long as the individual pulses in $Q_n(x)$ are well-separated, Lin's method produces a unique function $V(x)$ which solves \cref{PDEeigsystemper3} but which generically has $n$ jumps. These $n$ jumps are in the two-dimensional space spanned by $\Psi(0)$ and $\Psi^c(0)$, which gives us $2n$ jump conditions. Finding the eigenvalues amounts to solving the $2n$ jump conditions. As in \cite[Theorem 2]{Sandstede1998}, we will write these jump conditions in matrix form as a $2n \times 2n$ block matrix.

\subsection{Proof of \cref{nulambdalemmasimple}}

In this section, we locate the eigenvalue of $A(\lambda)$ which is close to 0 and characterize its eigenvector. We do the same for the adjoint $-A(\lambda)^*$.

...
\end{equation}
Then the substitutiton $U(x) = P_-(x; \lambda) Z(x)$ conjugates the ODE 
...
\begin{aligned}
U_i^-(x) &= d_i (\partial_x Q_i^-(x) + \lambda \partial_c Q_i^-(x)) + c_i V^-(x; \lambda) + W_i^-(x) && x \in [-X_{i-1}, 0] \\
U_i^+(x) &= d_i (\partial_x Q_i^+(x) + \lambda \partial_x Q_i^+(x)) + c_i V^+(x; \lambda) + W_i^+(x) && x \in [0, X_i] 
\end{aligned}
\end{equation}
where $c_i, d_i \in \C$ are constants, $W_i^-(x) \in C([-X_{i-1}, 0], \C)$, and $W_i^+(x) \in C([0, X_i], \C)$. Substituting this ansatz into \cref{PDEeigsystemper3} and simplifying, we obtain the following equation for $W_i^\pm(x)$.
\begin{align}\label{Wipm2}
(W_i^\pm)'(x) &= A(Q(x); \lambda) W_i^\pm(x) + G_i^\pm(x) W_i^\pm(x) + c_i G_i^\pm(x)V^\pm(x; \lambda) + d_i \lambda^2 \tilde{H}_i^\pm(x),
\end{align}
where
\begin{align*}
...
\begin{aligned}
(W_i^\pm)'(x) &= A(Q(x); \lambda) W_i^\pm(x) + G_i^\pm(x) W_i^\pm(x) + c_i G_i^\pm(x)V^\pm(x; \lambda) + d_i \lambda^2 \tilde{H}_i^\pm(x) \\
W_i^+(X_i) - W_{i+1}^-(-X_i) &= D_i d + C_i c && i = 0, \dots, n-1 \\
W_i^+(0) - W_i^+(0) &= c_i( V^-(0; \lambda) - V^+(0; \lambda) ) && i = 0, \dots, n-1  \\
\end{aligned}
\end{equation}
where
...
C_i c &= c_{i+1} V^-(-X_i; \lambda) - c_i V^+(X_i; \lambda)
\end{align}
...
We will now apply the conjugation we derived above to the system \cref{eigsystem}. Making the substitution $W_i^\pm(x) = P_\pm(x; \lambda) Z_i^\pm(x)$, we obtain the conjugated system
\begin{subequations}
\begin{align}
(Z_i^\pm(x))' = A(\lambda) Z_i^\pm(x) &+ P_\pm(x; \lambda)^{-1} G_i^\pm(x) P_\pm(x; \lambda) Z_i^\pm(x) \nonumber \\
&+ c_i P_\pm(x; \lambda)^{-1} G_i^\pm(x)V^\pm(x; \lambda) + \lambda^2 d_i P_\pm(x; \lambda)^{-1} \tilde{H}_i^\pm(x) \label{systemZ} \\
P_+(X_i; \lambda) Z_i^+(X_i) &- P_-(-X_i; \lambda) Z_{i+1}^-(-X_i; \lambda) = D_i d + C_i c \label{systemmiddle} \\
...
\begin{aligned}
\langle \Psi(0), P_+(0; \lambda) Z_i^+(0) - P_-(0; \lambda) Z_i^-(0) \rangle &= c_i \langle \Psi(0), V^+(0; \lambda) - V^-(0; \lambda) \rangle \\

...
As in \cite{Sandstede1998}, we will use Lin's method to construct eigenfunctions $V(x)$ to \cref{PDEeigsystemper2}. We will take a piecewise linear combination of kernel eigenfunctions as our ansatz, and then join these together with small remainder functions. Specifically, we proceed in the following manner.
...
\end{aligned}
\end{equation}
A key difference between our case and that considered in \cite{Sandstede1998} is that there are two generalized eigenfunctions in the kernel instead of one. Both of these kernel eigenfunctions satisfy the periodic boundary conditions. We will construct solutions $V(x)$ to \cref{PDEeigsystemper3} which are, to leading order, piecewise linear combinations of the kernel eigenfunctions $\partial_x Q_n(x)$ and $\partial_c Q_n(x)$. As long as the individual pulses in $Q_n(x)$ are well-separated, Lin's method produces a unique function $V(x)$ which solves \cref{PDEeigsystemper3} but which generically has $n$ jumps. These $n$ jumps are in the two-dimensional space spanned by $\Psi(0)$ and $\Psi^c(0)$, which gives us $2n$ jump conditions. (This is in contrast to \cite{Sandstede1998}, the jumps are in the one-dimensional space spanned by $\Psi(0)$). Finding the eigenvalues amounts to solving the $2n$ jump conditions.

\section{Piecewise formulation}

In this section, we write the eigenvalue problem \cref{PDEeigsystemper3} in a piecewise form similar to \cite{Sandstede1998}. Let $Q_n(x)$ be a periodic $n$-pulse solution to \eqref{genODE} constructed according to Theorem \ref{perexist}. From Theorem \ref{perexist} and Lemma \ref{solvewithjumps}, we can write $Q_n(x)$ piecewise as
...
To exploit the relations \eqref{PDEeigkernel}, we take the following piecewise ansatz for the eigenfunction $V(x)$
...
\end{aligned}
\end{equation}
where $d_i \in \C$ are constants, $W_i^-(x) \in C([-X_{i-1}, 0], \C)$, and $W_i^+(x) \in C([0, X_i], \C)$. Substituting this ansatz into \cref{PDEeigsystemper3} and simplifying, we obtain the following equation for $W_i^\pm(x)$.
...
For the eigenfunction $V(x)$ must be continuous, the $2n$ pieces $V_i^\pm(x)$ must also be joined together end-to-end in a loop. In other words, $n$ matching conditions at $x = \pm X_i$ and $n$ matching conditions at $x = 0$ must be satisfied. Thus to obtain an eigenfunction $V(x)$, the functions $W_i^\pm(x)$ must solve the system of equations
...
Before we can solve the system \cref{eigsystem} and derive the jump conditions \cref{jumpxi}, we will need a result regarding the eigenvalues and eigenvectors of the constant matrix $A(\lambda)$.
...
In this section, we state and prove the gap and conjugation lemmas. We will use the conjugation lemma to simplify the system of equations \cref{eigsystem} by converting $A(Q(x); \lambda)$ in to a constant coefficient matrix. The statements and notation of these lemmas are similar to those in \cite{Zumbrun2009}, except we allow the parameter vector $\Lambda$ to live in an arbitraty Banach space rather than in a subset of $\C^m$. The proofs follow those in \cite{Zumbrun2009}, but are given in much more detail. In addition, the proof of the conjugation lemma is used to prove a corollary. First, we state and prove the gap lemma.
...
In this section, we apply the conjugation lemma to the system of equations \cref{eigsystem}. This greatly simplifies the system by applying a change of coordinates that transforms the matrix $A(Q(x); \lambda)$ into the constant coefficient matrix $A(\lambda)$. For all $\lambda$, $A(Q(x); \lambda)$ decays exponentially to the constant-coefficient matrix $A(\lambda)$. Specifically, we have the exponential decay rate
\[
|A(Q(x); \lambda) - A(\lambda)| \leq C e^{-\alpha_0 |x|}
\]

Using the conjugation lemma on $\R^+$ with $\Lambda = \lambda$ and $\Lambda_0 = 0$, there exists $\delta_2 > 0$ and an invertible linear transformation 
\[
P_+(x; \lambda) = I + \Theta_+(x; \Lambda)
\]
such that for all $|\lambda| < \delta_2$, the change of coordinates $W_i^+ = P_+(x; \lambda) Z_i^+$ conjugates \eqref{Wipm2} into the equation
\begin{align*}\label{Zplus1}
(Z_i^+)'(x) &= A(\lambda) Z_i^+(x) + P_+(x; \lambda)^{-1} G_i^+(x) P_+(x; \lambda) Z_i^+(x) + d_i \lambda^2 P_+(x; \lambda)^{-1} \tilde{H}_i^+
(x)
\end{align*}
Note that we have only done this for the equations involving $W_i^+$. Furthermore, the conjugation operator $P_+(x; \lambda)$ has the form
\begin{equation}\label{projTheta}
P_+(x; \lambda) = I + \Theta_+(x; \lambda).
\end{equation}
$\Theta_+(x; \lambda)$ has the uniform decay rate
\begin{equation}\label{Thetadecay}
|\Theta_+(x; \lambda)| \leq C e^{-\alpha |x|} \\
\end{equation}
where $\alpha$ is slighly less than $\alpha_0$, and is the same $\alpha$ we chose in \cref{sec:existdichot}. From the conjugation lemma, these bounds also apply to derivatives with respect to $x$ and $\lambda$ (though with a different constant out front).
...
In the next lemma, we give an estimate for terms of the form $e^{\pm \nu(\lambda) X_j}$, which will show up in our analysis. In order to get a uniform estimate, we will place an upper bound on the  magnitude of $\lambda$. Although we can perform our analysis without doing this, this additional condition allows the form of the block matrix in Theorem \cref{blockmatrixtheorem} to relatively simple. 

\begin{lemma}\label{lemma:expnubound}
Choose any $N > 0$ and $\delta > 0$. Then there exists a constant $R = R(N, \delta)$ such that if $|\lambda| < \delta$ and $|\lambda| < N/|\log r|$,
...
\end{align*}
which is bounded for all $r \leq r_*$. Thus we conclude that there exists a constant $R$ depending on $N$ and $\delta$ such that the uniform bound \cref{expnubound} holds.
\end{proof}
\end{lemma}
...
\]
By Lemma \cref{lemma:Vcexists}, $|V^c(y) - V_0|\leq Ce^{-\alpha y}$. Since the first component of $V_0$ is $1/c$, $|v_0(y) - 1/c| \leq Ce^{-\alpha |x|}$. Using this with the third integral of \cref{Yintegform}, 
\begin{align*}
...
To conclude this section, as a corollary, we use the previous lemma to evaluate two important inner products.

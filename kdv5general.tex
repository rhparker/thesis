...
\subsection{Multi-pulse solutions}

A multi-pulse solution is a traveling wave solution that resembles multiple, well-separated copies of the primary pulse. From a spatial dynamics perspective, a multi-pulse is a multi-loop homoclinic orbit which remains near the primary pulse homoclinic orbit $Q(x)$. We will discuss the existence of multi-pulse solutions in [SECTION]. In addition, we will also consider periodic multi-pulses, which are periodic orbits composed of multiple loops which remain close to the homoclinic orbit $Q(x)$. The existence of periodic multi-pulses is discussed in [SECTION].

\section{Spectrum of pulses}

\subsection{PDE eigenvalue problem}\label{sec:PDEeig}

Our primary interest is the spectral stability of traveling wave solutions to \cref{genPDE}. For a traveling wave $\phi$, we will study the spectrum of the linearized operator $\partial_x \calL(\phi) = \partial_x(\calE''(\phi) + c)$. The PDE eigenvalue problem associated with $\phi$ is given by
...
Since $\phi$ is a traveling wave, $U(x)$ is exponentially localized, thus $A(U(x))$ is exponentially asymptotic to the constant coefficient matrix $A(0)$. The following lemma describes the eigenvalues and kernel of $A(0)$.
...
\noi Since $A(0)$ is not hyperbolic, the results of \cite{Sandstede1998} do not apply. Let $W^s(0)$, $W^u(0)$, and $W^c(0)$ be the stable, unstable, and center manifolds of the equilibrium at 0. These are not the same as in the previous section, but we will retain the notation for convenience. By reversibility, $\dim W^s(0) = m$ and $\dim W^u(0) = m$. The center manifold $W^c(0)$ is one-dimensional.

Let $q(x)$ be the primary pulse solution from \cref{sec:primarypulse}, and define $Q(x) \in R^{2m+1}$ by
\[
Q(x) = (q(x), \partial_x q(x), \dots, \partial_x^{2m-1} q(x), 0)
\]
We note that this is different from the primary pulse $Q(x)$ in \cref{sec:primarypulse} in that it has an extra component of 0 added to the end. Since it represents the same primary pulse solution, we retain the same notation for convenience. The variational and adjoint variational equations associated with \cref{PDEeigsystem} are
\begin{align}
V'(x) = A(Q(x)) V(x) \label{vareq2} \\
W'(x) = -A(Q(x))^* W(x) \label{adjvareq2}
\end{align}
where again we use the same notation $V$ and $W$ for convenience. Since $(\partial_x \calL(q))\partial_x q = 0$, it follows that $\partial_x Q(x)$ is an exponentially localized solution to the variational equation \eqref{vareq2}, i.e. 
\begin{equation}\label{Qprimevarsol}
(\partial_x Q(x))' = A(Q(x))(\partial_x Q(x))
\end{equation}
Since $(\partial_x \calL(q))(-\partial_c q) = \partial_x q$, we also have
...
\end{align*}
where $Y^+$ and $Y^-$ are different than in the previous section. Since $\dim \R Q'(0) \oplus Y^+ \oplus Y^- = 2m-1$, there are two remaining directions needed in order to span $\R^{2m+1}$. To find these, we will use the following lemma regarding solutions to \eqref{vareq2} and \eqref{adjvareq2}.
...
\begin{enumerate}
	...
	\end{equation}
	$V^c$ is symmetric with respect to the standard reversor operator $R$, i.e. $V^c(-x) = R V^c(x)$. Furthermore, $V^c = (\tilde{V}^c, 1)$, where $\tilde{V}^c$ solves $\tilde{V}^c(x)' = DF(Q(x)) \tilde{V}^c(x) + (0, \dots, 0, 1)^T$. Finally, if $v^c(x)$ is the first component of $V^c(x)$, $v^c$ is a formal solution to $\calL(q) v_c = 1$.
...
\noi We remark that although $[V^c(x)]' = A(Q(x))V^c(x)$, whether or not its first component $v^c(x)$ is in the kernel of $\partial_x \calL(q)$ depends on the space we use for the PDE eigenvalue problem \cref{PDEeigsystem}. The formal equation $\calL(q) v^c(x) = 1$ is very useful for computing $v^c(x)$ numerically.
...
We are interested in the spectrum of multi-pulse traveling wave solutions. For the spectrum of multi-pulses on $\R$, we will use an exponentially weighted space to obtain a similar result to that in \cite{Sandstede1998}. This method, however, only lets us find the spectrum to leading order. We do this in [SECTION]. For the spectrum of periodic multi-pulses, we pose the eigenvalue problem on the space of periodic functions. This is done in [SECTION].

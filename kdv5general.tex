...
\noi We remark that although $[V^c(x)]' = A(Q(x))V^c(x)$, whether or not $V^c(x)$ is an eigenfunction depends on the space we use for the PDE eigenvalue problem \cref{PDEeigsystem}. We also retain the same notation $\Psi(x)$ for convenience.

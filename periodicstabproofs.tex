...
\end{align*}
Since the integrals involving the stable and unstable subspaces have stronger bounds, wehave piecewise bounds
...
\]
For the integral terms, the bound is again determined by the integral involving the center subspace. This is similar to the corresponding integral in Lemma \ref{L1boundlemma} since the same estimates hold for $\tilde{H}_i^-$ and $G_i^-$. Putting all of this together, we have
\[
\| L_2(\lambda)_i^-(a,b,c,\tilde{c},d) \| \leq C(|a_{i-1}^-| + |b_i^-| + |e^{\nu(\lambda)X_{i-1}}c_{i-1}| + |c_{i-1}| + e^{\eta X_{i-1}}|\tilde{c}_{i-1}| + e^{-\alpha X_{i-1}}|\lambda|^2|d|)
\]

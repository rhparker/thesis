...
S(\lambda) = 
\begin{pmatrix}
K(\lambda) -\frac{1}{2} \lambda \tilde{M} K^+(\lambda) + C_1 & \lambda K_2(\lambda) - \lambda^2 \tilde{M}^c I + D_1 \\
-\frac{1}{2} \lambda M^c K^+(\lambda) + C_2 & A - \lambda^2 MI + D_2
\end{pmatrix}
\end{equation}
...
\end{pmatrix}
\end{equation}
where $\nu(\lambda)$ is defined in Lemma \ref{nulambdalemmasimple}. $K_1(\lambda)$ is the same matrix with all terms positive.
...
\end{align*}
where $v^c(y)$ is the first component of $V^c(y)$, which is defined in \cref{varadjsolutions}. NUMERICS STRONGLY SUGGESTS THAT $\tilde{M}^c = -M^c$. THIS IS AMAZING.
...
\begin{aligned}
E(\lambda) &= -2 \lambda^2 M \Bigg[ \left( 2a + \lambda^2 M + R_1 \right)\left( \sinh(\nu(\lambda)X) + \lambda \tilde{M} \cosh( \nu(\lambda X) \right) \\

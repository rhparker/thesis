...
A periodic multi-pulse is a multi-modal solution $q_n(x)$ to \cref{eqODE} on a periodic domain. Heuristically, we construct a periodic multi-pulse by gluing together multiple copies of the primary pulse end-to-end in a loop as shown in Figure \ref{fig:permultipulse}.
\begin{figure}
...
From a spatial dynamics perspective, an periodic multi-pulse is a multi-loop periodic orbit which remains close to the primary homoclinic orbit. A periodic $n$-pulse can be described by $n$ pulse distances $\{X_0, \dots, X_{n-1} \}$, where the distance between consecutive pulses in the loop is $2 X_j$. The period of the orbit is $2X$, where $X = X_0 + \dots + X_{n-1}$. A periodic $n$-pulse requires one more length parameter than a $n$-pulse on the real line, since we need one more connection to ``close the loop''.
...
\item $K_1(\lambda)$ and $K_2(\lambda)$ are perturbations of $K(\lambda)$ which have the same structure as $K(\lambda)$, but the entries are multiplied by $\mathcal{O}(|\lambda|(|\lambda| + e^{-\alpha X_m}))$ and $\mathcal{O}((|\lambda| + e^{-\alpha X_m})(|\lambda| + e^{-(\alpha - \eta) X_m}))$ (respectively). These are defined precisely in the proofs of Lemmas \ref{jumpcenteradj} and \ref{jumpadj}

...
Physically, a periodic $n$-pulse can described in terms of the $n$ pulse distances $\{X_0, \dots, X_{n-1} \}$, as shown in \cref{fig:permultipulse}. From a mathematical perspective, an alternative parameterization is more convenient and captures the underlying geometry necessary for a periodic $n$-pulse to exist. This parameterization is an adaptation of that in \cite{SandstedeStrut,Sandstede1998} to the periodic case. Let
\begin{equation}
\rho = \frac{\beta_0}{\alpha_0},
\end{equation}
where $\alpha_0$ and $\beta_0$ are defined in Hypothesis \ref{hypeqhyp} and are properties of the hyperbolic equilibrium at 0. Define the set
\begin{align}
...
For $m_0 \in \{0, 1\}$ and any positive integer $N$, there exists $r_* \in \mathcal{R}$ with $r_* > 0$ such that for all $r \leq r_*$:
\begin{enumerate}[(i)]
	\item There exists a family of symmetric periodic 2-pulses $Q_2(x; m_0, s_0, r)$ parameterized by $s_0 \in [0, 1)$. The distances $X_j$ are given by
	\begin{equation*}
		X_0(r, s_0) = X_1(r, s_0) = \frac{1}{2 \alpha_0} |\log r| + \frac{\pi}{2\beta_0} (m_0 + s_0) + L_0.
	\end{equation*}
	\item There exists a family of asymmetric periodic 2-pulses $\tilde{Q}_2(x; m_0, s_1, r)$ with distances $\tilde{X}_1 > \tilde{X}_0$ parameterized by $s_1 \in [0, N]$.	The distances $\tilde{X}_j$ are given by
	\begin{equation*}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation*}
	where $b_0(r; m_0, s_1)$ is smooth in both $r$ and $s_1$, and $b_0(0; m_0, k) = m_0$ for all odd integers $k$ between 1 and $N$.

	\item The two families meet at a pitchfork bifurcation. In terms of the two parameterizations, this occurs when $s_0 = p^*(r)$ and $s_1 = 0$. The bifurcation point $p^*(r)$ is smooth in $r$, and
	\[

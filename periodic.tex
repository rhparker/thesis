...
\end{align*}
where $L_0$ is a constant. The functions $\theta^*(\theta; m): [-p^*, \pi - p^*] \rightarrow \R$ are defined for all nonnegative integers $m$, are continuous in $\theta$, and have the following properties.
\begin{enumerate}[(i)]
...
\begin{enumerate}[(i)]
	...
	\]
	where $m_1$ is the largest nonnegative integer for which $s_1 = m_1 \pi + \theta$, with $\theta \in [-p^*, \pi - p^*)$. 
...
\begin{enumerate}[(i)]
	...
	\item For an asymmetric periodic 2-pulse $Q_2(x; m_0, s_1, r)$, $a = r \tilde{a}(r; m_0, s_1)$, where $\tilde{a}(r; m_0, s_1)$ is smooth in $r$. For all $s_1 > -p^*$,
	\begin{equation}
	\begin{aligned}
...
For any periodic parameterization $(m_0, \dots, m_{n-1}, \theta)$ with $\theta \notin \{-p^*, \pi - p^* \}$, let $r_* = r^*(m_0, \dots, m_{n-1}, \theta)$ be as in \cref{perexist}. Let $A$ be the matrix from \cref{blockmatrixtheorem}. Then for all $r \leq r_*$,
\[
A = r \tilde{A}(r)
\]

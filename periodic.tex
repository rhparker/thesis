...
\usepackage{xr} 
\externaldocument[ex:]{aux/periodicexist}
...
\item For any $r \in \mathcal{R}$ with $r < r_0$, there exists a unique $n$-periodic solution $U(x)$ to \eqref{genODE}. Bounds for the piecewise formulation \eqref{Upiecewise} are given in Lemma \ref{ex:solvewithjumps}.
...
b^*_j(m_j, 0) = b_j^0
\end{align}
Details on this parameterization are given in Lemma \ref{ex:thetaparamlemma}

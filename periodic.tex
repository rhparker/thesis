...
\end{hypothesis}
We remark that since we have a Hamiltonian system, the existence of an eigenvalue $\alpha_0 + \beta_0 i$ of $DF(0)$ implies the existence of the entire quartet $\pm \alpha_0 \pm \beta_0 i$.
...
E''(u^*(x)) v = \lambda v
\end{equation}
where $E''(u^*): H^{2m}(\R) \rightarrow H^{2m}(\R)$ is the Hessian of the Hamiltonian and is self-adjoint. Since $E'(u)$ is translation invariant, it follows that $E'(u^*(x)) \partial_x u^*(x) = 0$. We can write the eigenvalue problem \eqref{ODEeig} as the first-order system
...
\end{equation}
and $DF(u^*)$ is exponentially asymptotic to $DF(0)$, which is given by
\begin{equation}
DF(U^*) = \begin{pmatrix}
0 & 1 & 0 & \dots & 0 & 0 \\
0 & 0 & 1 & \dots & 0 & 0 \\
...
\end{pmatrix}
\end{equation}
The $c_i$ are constants which come from the linear part of $E'(u)$ and do not depend on $u^*$. By Hypothesis \ref{Eprimehyp}, no odd-order derivatives are involved in the linear part of $E'(u)$, thus $c_j = 0$ for $j$ odd. We make one additional assumption on the constant $c_0$, which we will need for the stability problem.
...
\end{equation}
where $Z = \R \Psi(0)$. Furthermore, $Z \perp \R Q'(0) \oplus Y^+ \oplus Y^-$.
...
U_i^+(x) &= Q^+(x; \beta_i^+) + V_i^+(x) && x \in [0, X_i]
\end{align}
where the subscripts $i = 0, \dots, n-1$ are taken $\mod n$. The functions $Q^\pm(x; \beta_i^\pm)$ are solutions to \eqref{genODE} with initial conditions $\beta_i^\pm$ in the stable/unstable manifold, and the functions $V_i^\pm$ are small remainder terms. Bounds are given in Lemma [].
\end{enumerate}
\end{theorem}

...
\begin{enumerate}[(i)] 
\item There exists an integer $M_0$ such that for all $m_1 \geq M_0$,
\begin{align}
...
&= (-1)^{m_0} \left( \sin \theta_0^*(\theta) - \rho \cos \theta_0^*(\theta) \right)
\end{align*}
...
&= \left| \sin \theta_0^*(\theta) - \rho \cos \theta_0^*(\theta) \right| \\
&\geq \left| |\sin \theta_0^*(\theta)| - \rho |\cos \theta_0^*(\theta)| \right| \\
&\geq \rho \left| |\cos \theta_j^*(\theta)| - \rho^{-1} |\sin \theta_j^*(\theta)| \right|
\end{align*}

For part (i), using the bound \eqref{thetastarbound}, we can find an integer $M_0$ such that if $m_1 \geq M_0$,
\[
|\sin \theta_0^*(\theta)| \leq K(\rho)
\]
where
\begin{align*}
K(\rho) &= \begin{cases}
...
\end{cases}
\end{align*}
Since $|\sin \theta_0^*(\theta)| \leq K(\rho) \leq 1/4$, it follows that $|\cos \theta_j^*(\theta)| \geq \sqrt{1 - K^2}|$. For $\rho \geq 1$,
\begin{align*}
| \partial_{b_0} G(b_0^*(m_0, \theta), b_1^*(m_1, \theta), 0) | 
&\geq \rho \left( \sqrt{1 - K^2}  - \rho^{-1}K \right) \\

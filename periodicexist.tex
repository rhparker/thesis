...
\subsection{Parameterization of zero set of $H$}
...
\begin{lemma}\label{Gderivbounds}
Fix $m_0 \in \Z$. Then for each integer $m_1$ with $m_1 \geq m_0$, let $(b_0^*(m_0, \theta), b_1^*(m_1, \theta))$ be the parameterization of the appropriate piece of the zero set of $H$ according to Lemma \ref{thetaparam}. Then we have the following bounds.
\begin{enumerate}[(i)] 
\item There exists an integer $M_0$ such that for all $m_1 \geq M_0$ and all $\theta \in I_\rho$,
\begin{align}\label{derivbound1}
| \partial_{b_0} G(b_0^*(m_0, \theta), b_1^*(m_1, \theta), 0) | &\geq \rho/2
\end{align}
...
| \partial_{b_0} G(b_0^*(m_0, \theta), b_1^*(m_1, \theta), 0) | &\geq \eta
\end{align}
\end{enumerate}
\begin{proof}
...
Taking the partial derivative of $G(b_0, b_1, r)$ with respect to $b_0$, we have
\begin{align*}
\partial_{b_0} G(b_0, b_1, r) = \sin(-\rho \log b_0) - \rho \cos(-\rho \log b_0) + \mathcal{O}(r^{\gamma/2\alpha})
\end{align*}
Evaluating this at $(b_0, b_1, r) = (b_0^*(m_0, \theta), b_1^*(m_1, \theta), 0)$, we have 
\begin{align*}
\partial_{b_0} G(b_0^*(m_0, \theta), b_1^*(m_1, \theta), 0) &= \sin\left(-\rho \log e^{-\frac{1}{\rho}(m_0 \pi - \theta_0^*(\theta))} \right) - \rho \cos\left(-\rho \log e^{-\frac{1}{\rho}(m_0 \pi - \theta_0^*(\theta))} \right) \\
...
For the $m = 1$, we would like to exclude a small neighborhood of the pitchfork bifurcation point. Choose any $\delta > 0$, and consider only $\theta \in I_\rho(\delta) = [-\theta_\rho^-, \theta_\rho^+ - \delta$. Since, from Lemma \ref{thetaparamlemma}, we know that $g(\theta)$ is increasing, we have
\begin{align*}
...
\end{align*}
Since the interval in \eqref{RHSboundgeq2} is contained in this interval, we conclude that we conclude that for all $m_1, \theta$ with either $m_1 \geq 2$ and $\theta \in I_\rho$ or $m_1 = 1$ and $\theta \in I_\rho(\delta)$,
\begin{align}\label{RHSbound2}
e^{ -\frac{1}{\rho} m_1 \pi } (-1)^{m_1} g(\theta) \in [-e^{-\frac{1}{\rho}\delta} T, e^{ -\frac{1}{\rho} \pi } T]
\end{align}
Recall that $g^{-1}$ is strictly increasing, and at the endpoints of its domain we have $g^{-1}(-T) = \theta_\rho^+$ and $g^{-1}(e^{\frac{1}{\rho}\pi}T) = \theta_\rho^+$. Since from \eqref{RHSbound2} we know that $e^{ -\frac{1}{\rho} m_1 \pi } (-1)^{m_1} g(\theta)$ lies strictly inside these endpoints, we conclude that there exists $\epsilon > 0$ (depending on $\delta$) such that for $m_1, \theta$ we are considering,
\begin{equation}\label{thetastarboundeps}
\theta_0^*(\theta) \in [\theta_\rho^- + \epsilon, \theta_\rho^+ - \epsilon]
\end{equation}

From the proof of Lemma \ref{pitchforkH}, 
\begin{align*}
G_{b_0}(b_0, b_1, 0) &= \sin \left( - \rho \log b_0 \right) - \rho \cos \left( - \rho \log b_0 \right)
\end{align*}
$G_{b_0}(b_0, b_1, 0)$ is smooth and is 0 if and only if $b_0 = p_k^*$. Using \eqref{thetastarboundeps}, for all $m_1, \theta$ we are considering,
\begin{align*}
b_0^*(m_0, \theta) 
...
\end{align*}
Since this closed interval lies strictly inside the interval $[p_{m_0}^*, \_{m_0+1}^*$, and $G_{b_0}(b_0, b_1, 0)$ is smooth and 0 only at the endponts, we conclude that there exists $\eta > 0$ (which depends on $\epsilon$, thus on $\delta$) such that for all $m_1, \theta$ that we are considering, 
\[
|\partial_{b_0} G(b_0^*(m_0, \theta), b_1^*(m_1, \theta), 0)| \eta
\]
...
H(b, b_{n-1}, \theta, r) = b - [D_b G(b_0, b_{n-1}, \theta, 0)]^{-1} G(b, b_{n-1}, \theta, r)
\end{equation}

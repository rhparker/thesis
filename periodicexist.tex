...
\noi This is the same as in \cite{SandstedeStrut,Sandstede1998} except for the addition of a phase parameter. The interval $[-\arctan \rho, \pi - \arctan \rho)$ for the phase parameter seems a little strange, but is chosen so that $\theta = 0$ corresponds the the baseline length parameters $b^0$. The pulse distances $X_i$ are determined by these parameters. For small $r$, the choice of scaling parameter $r = \exp\left(-\frac{2 k \pi}{\rho}\right)$ and baseline length parameter $b_j^0 = \exp\left(-\frac{ m \pi}{\rho}\right)$ corresponds to a pulse length of
\[
X_j \approx \frac{\pi}{2 b_0}(2 k + m) + L_0
\]
where $L_0$ is a constant. The phase parameter $\theta$ does not contribute significantly for small $r$.

The next theorem uses this parameterization to prove the existence of periodic multi-pulse solutions to \eqref{genODE}. The requirement that the scaling parameter $r$ be sufficiently small means that the individual pulses must be well-separated. 

[PUT A NICE PICTURE HERE. INCLUDE PITCHFORK TO EXPLAIN RESTRICTION THAT WE HAVE TO MAKE IN THE THEOREM WHICH SEEMS STRANGE].
...
\item This solution is specified by $n$ length parameters $b_j(r; m_j, \theta)$, where $b_j(0; m_j, 0) = b_j^0$. Details on this parameterization are given in Lemma \ref{thetaparamlemma}.

\item The $n$ pulses in $U(x)$ are separated by distances $2 X_0, \dots, 2 X_{n-1}$, where 
\begin{equation}\label{Xj}
X_j = -\frac{1}{2\alpha_0}\log(b_j(r; m_j, \theta) r) - \frac{\phi}{2 \beta_0} 
\end{equation}
...
X = \frac{1}{2\alpha_0} \left(n |\log r| + |\log(b_0(r; m_0, \theta) \cdots b_{n-1}(r; m_{n-1}, \theta))| \right) - \frac{n \phi}{2 \beta_0}
\end{align}
...
Finally, we show that for the the family of periodic 2-pulses, solutions with unequal pulse distances bifurcate from solutions with equal pulse distances in a series of pitchfork bifurcations.

\begin{theorem}\label{2pulsebifurcation}
Assume Hypotheses \ref{Ehyp}, \ref{Hhyp}, \ref{hypeqhyp}, \ref{Qexistshyp}, and \ref{H0transversehyp}. Let $Q(x)$ be a transversely constructed, symmetric primary pulse solution to \eqref{genODE}. Then for sufficiently small $r$, asymmetric 2-periodic solutions bifurcate from symmetric 2-periodic solutions in a series of pitchfork bifurcations. The locations of these pitchforks are given below in \cref{pitchpersist}.
\end{theorem}
...
and for $\theta = 0$,
\[
(b_0^*(m_0, 0), b_1^*(m_1, 0)) = (b_0^0, b_1^0)
\]. 
In addition, these families of solutions have the following properties.
...
\end{equation}
where $p_0^*$ is the pitchfork bifurcation point defined in Lemma \ref{pitchforkH}, and 
\[
b_0(0, 0) = b_1(0, 0) = b_0^0 = 1
\]
...
\begin{enumerate}
	...
	\]
	Then we can rewrite equation \eqref{thetastareq} as
	...
	Furthermore, we can easily show that the only zero of $g(\theta)$ on $I_\rho$ is as $\theta = 0$.
	...
	\end{equation}
	Since $g$ is strictly increasing on $I_\rho$, $g$ is invertible on $[-T, e^{\frac{1}{\rho}\pi} T]$ and $g^{-1}: [-T, e^{\frac{1}{\rho}\pi} T] \rightarrow I_\rho$ is strictly increasing, with $g^{-1}(-T) = \theta_\rho^-$ and $g^{-1}(e^{\frac{1}{\rho}\pi} T) = \theta_\rho^+$. Since $g(0) = 0$, $g^{-1}(0) = 0$.

	\item Let
	\[
	t(\theta, m_1) = e^{ -\frac{1}{\rho} m_1 \pi } (-1)^{m_1} g(\theta)
	\]
	be the RHS of \eqref{thetastareq2}. It follows from \eqref{grange}
	...
	Since $m_1 \geq 1$, 
	\begin{equation*}
	t(\theta, m_1) \in [-T, e^{-\frac{1}{\rho}\pi} T]
	\end{equation*}
	...
	\end{equation}
	Since $\theta^*(\theta) \in I_\rho$, this ensures that we cannot overlap the region parameterized by a different $b_0^0$ by varying $\theta$. 

	Furthermore, if $m_1 = 1$ and $\theta = \pi - \arctan \rho$, then $t(\theta, m_1) = -T$. For all $m_1$ and $\theta \in I_\rho$ other than this,
	\[
	t(\theta, m_1) \in \left(-T, e^{-\frac{1}{\rho}\pi} T\right]
	\]
	Since $g^{-1}$ is strictly increasing and $g(0) = 0$, this implies that $|\theta^*(\theta, m_1) \leq |\theta|$, with equality when $m_1 = 1$ and $\theta = \pi - \arctan \rho$ and strict inequality otherwise.

	\item Finally, we will show what happens at $\theta = 0$. Since $g(0) = 0$, $\theta^*(0) = 0$, thus $b_0^*(m_0, 0) = b_0^0$. $b_1^*(m_1, 0) = b_1^0$ by definition.

\end{enumerate}

\item For each $m_1$, equations \eqref{thetaparam1} and \eqref{solvethetastar} give us a smooth parameterization in $\theta$. We will now show that the pieces match up at their endpoints.  

\begin{enumerate}
	\item Show that the pieces which do not involve the pitchfork bifurcation match up. Let $m_1 \geq 1$, so that we avoid the pitchfork bifurcation point $p_0^*$. Let $(b^*_0(0, \theta), b^*_1(m_1, \theta))$ and $(\tilde{b}^*_0(0, \theta), \tilde{b}^*_1(m_1 + 1, \theta))$ parameterize adjacent pieces. For the second components,
	\begin{align*}
	...
	\end{align*}
	Thus the second components match at $p^*_{m_1}$. For the first components, let $b_0^*(0, \theta) = e^{ \frac{1}{\rho} \theta_0^*(\theta) }$ and $\tilde{b}_0^*(0, \theta) = e^{ \frac{1}{\rho} \tilde{\theta}_0^*(\theta) }$. From \eqref{solvethetastar}, $\theta_0^*$ and $\tilde{\theta}_0^*$ are given by
	\begin{align*}

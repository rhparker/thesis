...
Symmetric solutions with $b_0 = b_1$ exist by Lemma \ref{Gsymm}. To paramaterize these, for $m_0 \in \{0, 1\}$ and $s_0 \in [0, \pi)$, let
\[
b_0(m_0, s_0) = b_2(m_0, s_0) = 
\exp\left[ -\frac{1}{\rho}(m_0 \pi + s_0) \right]
\]
Then the pulse distances are given by \cref{2psymmdist}. 

By Lemma \ref{pitchforkconnects}, for $m_0 = 0$, a pitchfork bifurcation point occurs on the diagonal at $(p_0^*(r), p_0^*(r))$. Similarly, for $m_0 = 1$, a pitchfork bifurcation point occurs on the diagonal at $(p_1^*(r), p_1^*(r))$. 

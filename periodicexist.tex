...
t_j(r; m_j, m_{n-1}, \theta) = -\rho \frac{1}{\rho}\log b_j(r)
\end{equation}
...
In the next lemma, we show that the pitchfork bifurcations persists on the diagonal for sufficiently small $r$.

% lemma :Persistence of pitchfork

\begin{lemma}\label{pitchpersist}
For $k \in \Z$ there exists $r_2 > 0$ (depending on $k$) and a unique smooth function $b_k^*: \mathcal{R} \cap [0, r_2] \rightarrow \R$ such that $G(b_0, b_1, r)$ has a non-degenerate pitchfork bifurcation at $(b_k^*(r),b_k^*(r))$, and 
\begin{equation*}
b_k^*(r) \rightarrow b_k^* \text{ as } r \rightarrow 0
\end{equation*}
\begin{proof}
For simplicity, take $k = 0$. The same argument works for all other $k$. In Lemma \ref{pitchforkH}, we showed that a pitchfork bifurcation occurs in the zero set of $H(b_0, b_1)$ at $(b_0^*, b_0^*)$. As in Lemma \ref{pitchforkH}, we make the change of coordinates $(b_0, b_1) \mapsto (x, y)$, so that $(0, b^*)$ is a pitchfork bifurcation point of $G(x, y, 0)$. By Lemma \ref{Gsymm}, we have the symmetry 
\[
G(-x, y, r) = -G(x, y, r)
\]
...
Let $p_k^*(r) = -\rho \log(b_k^*(r)) - k \pi$. Then
\begin{equation}
p_k^*(r) \rightarrow p^* \text{ as }r \rightarrow 0
\end{equation}

In Lemma \ref{Gsymm}, we showed that for sufficiently small $r$, $G(b_0, b_0, r) = 0$, so the zero set of $G$ contains the diagonal. In the next lemma, we show that the arms of the pitchfork also persist for sufficiently small $r$. It suffices to show this when $m = 0$, i.e. for the pitchfork at $(b_0^*(r), b_0^*(r))$. By symmetry, we also only need to show it for the lower arm. 

\begin{lemma}\label{armpersists}
Choose any $\delta > 0$. Then there exists $r_3 < 0$ such that for $r < r_3$, the portion of the zero set of $G(b_0, b_1, r)$ corresponding to lower arm of the pitchfork at $(b_0^*(r), b_0^*(r))$ is parameterized by
\begin{align*}
(b_0, b_1) = (\tilde{b}_0(s; r), \tilde{b}_1(s, r)) && s \in [p^* + \delta, \infty)
\end{align*}
where
...
\begin{proof}
For $r = 0$, we use the parameterization from Lemma \ref{thetaparamlemma}. Take $m_0 = 0$. For every positive integer $m_1$ with $m_1 \geq 1$, as in Lemma \ref{thetaparamlemma}, let
\begin{equation}\label{thetaparam}
\begin{aligned}
b_0( 0, m_1, \theta) &= \exp\left[ -\frac{1}{\rho}\theta^*(\theta, m_1 - m_0) \right] \\
b_1( 0, m_1, \theta) &= \exp\left[ -\frac{1}{\rho}(m_1 \pi + \theta) \right]
\end{aligned}
\end{equation}
Since these families connect together at their endpoints, define $\tilde{b}_1(s)$ by \cref{tideb1}. For any $s > p^*$, let $m(s)$ be the largest integer $m$ such that $m \pi \leq s - p^*$, and let $\theta(s) = s - (m(s) + 1)\pi$. Then we define
\[
\tilde{b}_0(s) = b_0(0, m(s)+1, \theta)
\]
Thus the continuous curve 
\begin{align*}
(b_0, b_1) &= (\tilde{b}_0(s), \tilde{b}_1(s)) && s > p^*
\end{align*}
parameterizes the lower arm of the pitchfork corresponding to $m_0 = 0$ when $r = 0$. We will show that this curve persists for small $r$.

For $\delta > 0$, let $X$ be the Banach space
\[
...
By Lemma \ref{Hderivboundslemma}, there exists $\eta > 0$ (depending on $\delta$) such that for all $s \geq p^* + \delta$,
\begin{align*}
...
\[
[D_{b_0} \tilde{G}(\tilde{b}_0, 0)]^{-1} \leq \frac{1}{\eta}
\]
...
In the final lemma, we show that for sufficiently small $r$, the lower arm of the pitchfork connects to the pitchfork bifurcation point. This will extend the parameterization in Lemma \ref{armpersists} to $s \in [0, M \pi]$.

\begin{lemma}\label{pitchforkconnects}
There exists $r_4 > 0$ such that for $r \leq r_4$, the lower arm of the pitchfork at $(b_0^*(r), b_0^*(r))$ is parameterized by
\begin{align*}
(b_0, b_1) = (\tilde{b}_0(s; r), \tilde{b}_1(s)) && s \in [p^*, \infty)
\end{align*}
where
\begin{align*}
\tilde{b}_1(s) &= e^{-\frac{1}{\rho}s} 
...
and 
\begin{align*}
(\tilde{b}_0(p^*; r), \tilde{b}_1(p^*)) = (b_0^*(r), b_0^*(r))
\end{align*}
...
y &= \frac{1}{2}(b_1 + b_0)
\end{align*}
When $r = 0$, there is a pitchfork bifurcation at $(b_0^*, 0)$. Then there exists $y_1 > 0$ such that the lower arm of the pitchfork is parameterized in the $(x,y)$ coordinate system by 
\begin{align*}
(x, y) &= (x_0^*(y), y) && y \in [0, y_1]
\end{align*}
where $x_0^*(0) = b_0^*$. 

In Lemma \ref{pitchpersist}, we showed that this pitchfork bifurcation persists for small $r$. Specifically, there exists $r_2 > 0$ and $y_2$ with $0 < y_2 \leq y_1$ such that for all $r < r_2$, there is a pitchfork bifurcation at $(0, b_0^*(r))$, and the lower arm of the pitchfork is uniquely parameterized by
\begin{align*}
(x, y) &= (x_0(y, r), y) && y \in [0, y_2]
\end{align*}
with $x_0(0, r) = b_0^*(r)$ and $x_0(y, 0) = x_0^*(y)$. 

Take $\delta = y_2/2$ in Lemma \ref{armpersists}. Converting the parameterization to the $(x, y)$ coordinate system, there exists $r_3 > 0$ such that the lower arm of the pitchfork on the interval $[y_2/2, y_2]$ is uniquely parameterized by 
\begin{align*}
(x, y) &= (x_1(y, r), y) && y \in \left[\frac{y_2}{2}, y_2\right]
\end{align*}

Let $r_4 = \min\{ r_2, r_3 \}$. Then by uniqueness of the two parameterizations, for $r \leq r_4$,
\begin{align*}
...
Symmetric solutions with $b_0 = b_1$ exist by Lemma \ref{Gsymm}. By Lemma \ref{pitchforkconnects}, a pitchfork bifurcation point occurs on the diagonal at $(p_0(r), p_0(r))$. At this point, asymmetric solutions bifurcate from symmetric solutions.

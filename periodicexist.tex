...
\end{align*}2
where $(p^*_m, p^*_m)$ is the pitchfork bifurcation point from Lemma \ref{pitchforkF} and $m_1 \geq m_0$.
...
\begin{proof}
First, since we are taking $b_0^0 \geq b_0^1$, i.e. $m_0 \leq m_1$, it suffices to consider the case where $b_0^0 = 1$, i.e. $m_0 = 0$. To see this, suppose the lemma is true for $b_0^0 = 0$. Then we can find a family of solutions $(b_0^*(0, \theta), b_1^*(m_1 - m_0, \theta)$ to $H(b_0, b_1) = 0$. It follows that $(e^{-m_0 \pi/\rho} b_0^*(0, \theta), e^{-m_0 \pi/\rho} b_1^*(m_1 - m_0, \theta))$ solves $F(b_0, b_1) = 0$, and 
\begin{align*}
e^{-m_0 \pi/\rho} b_0^*(0, 0) &= b_0^0 \\
e^{-m_0 \pi/\rho} b_1^*(m_1 - m_0, 0) &= b_0^1
\end{align*}

Take $b_0^0 = 1$. For $\theta \in I_\rho$, take the parameterization
...
\end{aligned}
\end{equation}
so that $b_1^*(m_1, 0) = b_1^0$. The function $\theta_0^*(\theta)$ will be dertrmined below. Since we need to parameterize two pieces of a pitchfork, we will need to consider the cases $b_1^0 = 1$ and $b_1^0 > 1$ separately.
\begin{enumerate}
	\item Let $b_1^0 = 1$, so that $b_1^*(0, \theta) = e^{ \frac{1}{\rho}\theta}$. Then we can obtain a solution to $H(b_0, b_1) = 0$ by taking $\theta_0^*(\theta) = \theta$, so that $b_0^*(0, \theta) = e^{ \frac{1}{\rho}\theta}$. Thus we have the solution
	\[
	...
	\item Now consider the case $b_1^0 \neq 1$ so that $m_1 > 0$. Plugging the expressions eqref{\thetaparam1} for $b_1^*(m_1, \theta)$ and $b_0^*(0, \theta)$ into $H$ to get
	\begin{align*}
	...
	\begin{enumerate}
		...
		The only critical point of $g'(\theta)$ on $I_\rho$ is a local maximum at $\theta = \arctan \rho$. Since $g'(\theta) > 0$ on $(-\arctan \rho,\pi - \arctan \rho)$ and is zero at the endpoints, it follows that $g(\theta)$ is strictly increasing on $I_\rho$.
		...
		\item It follows from \eqref{grange} that the RHS of \eqref{thetastareq1} has bounds
		...
		\end{equation*}
		i.e. the RHS of \eqref{thetastareq1} lies is contained in thedomain of $g^{-1}$. Thus for $\theta \in I_\rho$, we can solve for $\theta^*$ in terms of $\theta$, i.e. 
		...
		\item For each $m_1$, we have a smooth parameterization in $\theta$. We will now show that the pieces all match up. First, we consider the case where $m_1 \geq 1$, so that we avoid the pitchfork bifurcation point $p_0^*$. Let $(b^*_0(0, \theta), b^*_1(m_1, \theta))$ and $(
		\tilde{b}^*_0(0, \theta), \tilde{b}^*_1(m_1 + 1, \theta))$ paramaterize adjacent pieces. For the second component, we have
		\begin{align*}
		b_1(\theta_\rho^-) &= e^{ -\frac{1}{\rho}(m_1 \pi + \arctan \rho) } \\
		\tilde{b}_1(\theta_\rho^+) 
		...
		and
		\begin{align*}
		\tilde{\theta}^* &= g^{-1}\left( e^{ -\frac{1}{\rho} (m_1+1) \pi } (-1)^{m_1+1}  e^{ \frac{1}{\rho} \theta } \sin(\pi - \arctan \rho) \right)
		\end{align*}

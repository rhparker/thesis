...
\| L_2(\lambda) (a,b,c,\tilde{c},d) \| \leq C\left(|a| + |b| + e^{\eta X_* }(|c| + |\tilde{c}|) + e^{-\alpha X_*}|\lambda|^2|d|\right)
\end{equation}
\begin{proof}
For the ``minus'' pieces, since $\Phi^c(x, -X_{i-1}; \lambda) = e^{\nu(\lambda)(x + X_{i-1})}$, the terms involving $c_{i-1}^-$ and $\tilde{c}_{i-1}^-$ have bounds
\begin{align*}
\| \Phi^c(x, -X_{i-1}; \lambda) \nu(\lambda)(c_{i-1} - \tilde{c}_{i-1}) \| \leq C e^{-\eta X_*}(|c| + |\tilde{c}|).
\end{align*}
...
\| Z_1(\lambda)(a,b,c,\tilde{c},d) \| \leq C\left(|a| + |b| + e^{\eta X_* }(|c| + |\tilde{c}|) + e^{-\alpha X_*}|\lambda|^2|d|\right)
\end{equation}
We also have the piecewise bounds
\begin{equation}\label{Z1piecewise}
\begin{aligned}
\| Z_1(\lambda)_i^-(a,b,c,\tilde{c},d)(x) \| \leq C\left(|a| + |b| + e^{\eta (X_{i-1} + x) }(|c| + |\tilde{c}|) + e^{-\alpha X_*}|\lambda|^2|d|\right) \\
\| Z_1(\lambda)_i^+(a,b,c,\tilde{c},d)(x) \| \leq C\left(|a| + |b| + e^{\eta (X_i - x) }(|c| + |\tilde{c}|) + e^{-\alpha X_*}|\lambda|^2|d|\right) 
\end{aligned}
\end{equation}
\begin{proof}
...
\end{equation}
The bound \eqref{Z1bound} follows from the bound \eqref{L2bound} from Lemma \ref{L2boundlemma}. The piecewise bounds are obtained by plugging the uniform bound \cref{Z1bound} into the fixed point equations \cref{Zfpeq} and bounding the integrals involving $Z$ as in \cref{L1boundlemma}.
\end{proof}
\end{lemma}
...
\| Z_2(\lambda)(a,b,c,\tilde{c},d) \| \leq C\left(|a| + |b| + e^{\eta X_* }(|c| + |\tilde{c}| + |D_i||d|) + e^{-(\alpha-\eta) X_*}|\lambda|^2|d|\right)
\end{equation}
...
&\leq C \left(|a_i^+| + |b_i^+| +  e^{-\alpha X_i}|\lambda|^2|d| \right) \int_0^{X_i} e^{-\alpha(X_i - y)} |G_i^+(y)| dy \\
&\qquad+ (|c_i| + |\tilde{c}_i|) \int_0^{X_i} e^{-\alpha(X_i - y)} e^{\eta(X_i - y)}|G_i^+(y)|dy
\end{align*}
For the second integral, using the estimates on $G$ from Lemma \ref{stabestimateslemma}, we have
\begin{align*}
\int_0^{X_i} &e^{-\alpha(X_i - y)} e^{\eta(X_i - y)} |G_i^+(y)| dy \leq C \int_0^{X_i} e^{-(\alpha-\eta)(X_i - y)} \left(e^{-\alpha_1(X_i - y)}e^{-\alpha_1 X_i} + e^{-2 \alpha_1 X_{i-1}} e^{-\alpha_1 y} \right) dy \\
&\leq C \left( e^{-\alpha_1 X_i} \int_0^{X_i} e^{-(\alpha + \alpha_1 - \eta)(X_i - y)} dy + e^{-2 \alpha_1 X_{i-1}} \int_0^{X_i} e^{-(\alpha - \eta)(X_i - y)}e^{-\alpha_1 y} dy \right) \\
&\leq C \left( e^{-\alpha_1 X_i} + e^{-2 \alpha_1 X_{i-1}} \right)\\
&\leq C e^{-\alpha_1 X_*}
\end{align*}
The first integral can be bounded by this well. Thus we have
\begin{align*}
&\left|
P^+(X_i; \beta_i^+, \lambda) \int_0^{X_i} \Phi^s(X_i, y; \lambda) P_+(y; \lambda)^{-1} G_i^+(y) P_+(y; \lambda) Z_i^+(y) dy \right| \\
&\qquad \leq C e^{-\alpha_1 X_*} \left(|a| + |b| + |c| + |\tilde{c}| + e^{-\alpha X_i}|\lambda|^2|d| \right) 
\end{align*}
...
\begin{align}
|A_2(\lambda)_i(b, c, d)|
&\leq C \left(e^{-\alpha X_i}(|b| + |c|) + e^{-\alpha_1 X_i}|\lambda|^2|d| + e^{-\alpha X_m}|D_i||d| \right) \label{A2bound}
\end{align}
...
\begin{align*}
|A_2&(\lambda)_i(b, c, d)| \leq C \left( e^{-\alpha X_*} |b| + e^{-\alpha_1 X_*} (|c| + |\tilde{c}| + |\lambda|^2 |d| + |D_i||d|) \right)
\end{align*} 
...
In the next lemma, we derive an expression for $\tilde{c}_i$. We then use it to obtain an improved bound for $Z_2(\lambda)_i(b,c,d)$.
...
|C_2(\lambda)(b, c, d) \leq C |\lambda| \left( e^{-\alpha X_*} |b| +  e^{-\alpha_1 X_*}(|c| + |\lambda|^2 |d| ) + |D_i||d| \right)
\end{align}
In addition, we have the following improved bound for $Z_2(\lambda)$.
\begin{equation}\label{Z2bound2}
\| Z_2(\lambda)(a,b,c,\tilde{c},d) \| \leq C\left(|b| + e^{\eta X_* }|c| + e^{-\alpha X_*}|\lambda||d| + |D_i||d| \right)
\end{equation}
and the following piecewise bounds for $Z_2(\lambda)_i^-(x)$
\begin{equation}\label{Z2piecewise}
\begin{aligned}
\| Z_2(\lambda)_i^-(a,b,c,\tilde{c},d)(x) \| \leq C\left(|b| + e^{\eta (X_{i-1} + x) }|c| + e^{-\alpha X_*}|\lambda||d| + |D_i||d| \right) \\
\| Z_2(\lambda)_i^+(a,b,c,\tilde{c},d)(x) \| \leq C\left(|b| + e^{\eta (X_i - x) }|c| + e^{-\alpha X_*}|\lambda||d| + |D_i||d|\right) 
\end{aligned}
\end{equation}
...
\end{align*}
Substituting the bound $A_1$ for $|a|$, this becomes
\begin{align*}
|P^c(\lambda)\tilde{L}_3&(\lambda)_i(a,b,\tilde{c},c,d)| \leq C |\lambda| e^{-\alpha_1 X_*}  \left( |b| + |c| + e^{-\alpha_1 X_*} |\lambda^2| |d| + |D_i||d| \right)
\end{align*}
...
\end{align*}
Using the bound $A_1$ for $\tilde{c}$ on the RHS and the bound for $\tilde{L}_3$, dividing by 2, and simplifying, we get
\begin{align}\label{ciexp1}
\tilde{c_i} &= \lambda q(X_i) (d_{i+1} - d_i ) + C_2(\lambda)_i(b, c, d)
\end{align}
where $C_2$ has the bound \cref{C2bound}. Finally, we substitute $A_1$ from Lemma \ref{Zinv1} and \cref{ciexp1} for $\tilde{c}$ into $Z_1$ from Lemma \ref{Zinv0} to get the improved bound \cref{Z2bound2} for $Z_2(\lambda)$, where we used the bound $|D_i| \leq C e^{\alpha_1 X_*}$. The piecewise bounds \cref{Z2piecewise} are obtained by plugging the improved uniform bound \cref{Z2bound}, \cref{A1bound}, and \cref{ciexp1} into the fixed point equations \cref{Zfpeq}.
\end{proof}
\end{lemma}
...
\item For the $b_i$, since the conjugation operators $P_\pm(x; \lambda)$ and eigenprojections $P_0^{s/u}(\lambda)$ are smooth in $\lambda$, we have
\begin{align*}
|(P_-(0; \lambda) &- P_-(0; 0))b_i^- + P_-(0; \lambda)(P_0^u(\lambda) - P_0^u(0))b_i^-| \leq C |\lambda| |b|
\end{align*}
The $b_i^+$ term is similar. 

\item For the $c_i$ terms, similarly to the $b_i$ terms, we have
\begin{align*}
|(P_-(0; \lambda) &- P_-(0; 0)) P_0^c(0) e^{\nu(\lambda) X_{i-1}} c_{i-1} + P_-(0; \lambda) (P_0^c(\lambda) - P_0^c(0)) e^{\nu(\lambda) X_{i-1}} c_{i-1} | \leq C |\lambda||e^{\nu(\lambda) X_{i-1}} c_{i-1} |
\end{align*}
The term involving $c_i$ is similar.

\item The $\tilde{c}_i$ terms are also handled  similarly to the $b_i$. We use the expression for $\tilde{c}_i$ from Lemma \ref{lemma:tildec1} and the bound on $C_2$ to get
\begin{align*}
|(P_+&(0; \lambda) - P_+(0; 0))P_0^c(0) e^{-\nu(\lambda)X_i} \tilde{c}_i + P_+(0; \lambda) (P_0^c(\lambda) - P_0^c(0)) e^{-\nu(\lambda)X_i} \tilde{c}_i| \\
&\leq C |\lambda|^2 e^{\eta X_*} \left( e^{-\alpha X_*} |b| +  e^{-\alpha_1 X_*}(|c| + |\lambda|^2 |d| ) + |D_i||d| \right) \\
&\leq C |\lambda|^2 \left( e^{-(\alpha - \eta) X_*} |b| +  e^{-\alpha X_*}(|c| + |d|) \right) 
\end{align*}
...
\item The bound on the integral terms is determined by the integrals involving the center subspace, since the other integrals will have stronger bounds. For the integrals involving $Z$, we use the bound on $G_i^\pm$ from Lemma \ref{stabestimateslemma} to get
\begin{align*}
&\left| P_i^+(0; \lambda) \int_{X_i}^0 \Phi^c(0, y; \lambda) P_+(y; \lambda)^{-1} G_i^+(y) P_+(y; \lambda) Z_i^+(y) dy \right| \\
&\leq C \int_0^{X_i} e^{\eta y} \left( e^{-\alpha_1 X_i} e^{-\alpha_1(X_i - y) } + e^{-2 \alpha_1 X_{i-1}} e^{-\alpha_1 y} \right) | Z_i^+(y) | dy \\
\end{align*}
For the first integral, we use the piecewise bound on $Z_2$ from Lemma \ref{lemma:tildec1} to get
\begin{align*}
\int_0^{X_i} &e^{\eta y} e^{-\alpha_1 X_i} e^{-\alpha_1(X_i - y) } dy \\
&\leq C \left( \left( |b| + e^{-\alpha X_*}|\lambda||d| + |D_i||d|\right) \int_0^{X_i} e^{\eta y} e^{-\alpha_1 X_i} e^{-\alpha_1(X_i - y) } dy + |c| \int_0^{X_i} e^{\eta y} e^{-\alpha_1 X_i} e^{-(\alpha_1 - \eta) (X_i - y) } dy \right) \\
&\leq C e^{-\alpha X_*}\left( |b| + |c| + e^{-\alpha X_*}|\lambda||d| + |D_i||d|\right) 
\end{align*}
For the second integral, we use the uniform bound on $Z_2$ from Lemma \ref{lemma:tildec1} to get
\begin{align*}
\int_0^{X_i} &e^{\eta y} e^{-2 \alpha_1 X_{i-1}} e^{-\alpha_1 y} | Z_i^+(y) | dy \\
&\leq C\left(|b| + |c| + e^{-\alpha X_*}|\lambda||d| + |D_i||d|\right) e^{-2 \alpha_1 X_*} e^{\eta X_*} \int_0^{X_i} e^{-(\alpha_1 - \eta) y} dy \\
&\leq C e^{-\alpha X_*}\left( |b| + |c| + e^{-\alpha X_*}|\lambda||d| + |D_i||d|\right) 
\end{align*}
The other integral term is similar.

\item For the integrals involving $\tilde{H}_i^\pm$, we have
\begin{align*}
\left| \lambda^2 d_i P_-(0; \lambda) \int_{-X_{i-1}}^0 \Phi^c(0, y; \lambda) P_-(y; \lambda)^{-1} \tilde{H}_i^-(y) dy \right| &\leq C |\lambda|^2 |d| \int_{-X_{i-1}}^0 e^{-\eta y} e^{\alpha_1 y} dy \leq C |\lambda|^2 |d|
\end{align*}
We could obtain a better bound, but it is not necessary. The other term is similar.
\end{enumerate}

Combining all of these individual bounds together, using the bound $|D_i| \leq C e^{-\alpha_1 X_*})$, and simplifying, we obtain the bound for $L_4(\lambda)(b, c, d)$.
\begin{align*}

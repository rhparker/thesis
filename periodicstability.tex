...
Finally, let
\begin{align*}
A(x; \lambda) &= A(Q(x)) + \lambda B \\
\tilde{H}_i^\pm(x) &= \partial_c Q_i^\pm(x) \\ 
H(x) &= \partial_c Q(x)
\end{align*}
Using these, equation \eqref{Wipm1} simplifies to
\begin{align*}
(W_i^\pm)'(x) &= A(x; \lambda) W_i^\pm(x) + G_i^\pm(x) W_i^\pm(x) + d_i \lambda^2 \tilde{H}_i^\pm(x)
\end{align*}
...
To simplify the system, we would like to apply a change of coordinates so that the linear operator $A_i^\pm(x; \lambda)$ is transformed into a constant coefficient matrix. To do this we will use the Conjugation Lemma. For all $\lambda$, $A^\pm(x; \lambda)$ decays exponentially to the constant-coefficient matrix $A(0; \lambda)$. For convenience, let $A(\lambda) = A(0; \lambda)$. Specifically,
\[
|A^\pm(x; \lambda) - A(\lambda)| \leq C e^{\alpha_0 |x|}
\]

Let $\Lambda = (U(x), \lambda) \in C_b(\R^+) \times \C$. $C_b(\R^+)$ is the space of bounded, continuous functions on $\R^+$ equipped with the supremum norm, and is known to be a Banach space. Let $\Lambda_0 = (Q(x), 0)$. Write the matrix $A(U(x); \lambda)$ as $A(x; \Lambda) = A(x; U(x), \lambda)$, where both $\lambda$ and the function $U(x)$ are parameters. Then 

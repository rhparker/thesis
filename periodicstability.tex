...
To conclude this section, we will derive an expression for the center projections $\tilde{P}^c_\pm(x)$. From \cref{trichotomyprojunconj}, it follows that
\begin{align*}
...
Since $\Psi^(x) = W_0$ is a constant solution to the adjoint variational equation, we can remove the conjugation operator from this expression. [I AM ALMOST CERTAIN THIS IS POSSIBLE, BUT THE PROOF IS COMPLETE.]
...
\end{align}
\begin{proof}
We will prove the result on $\R^+$. The result on $\R^-$ is similar. First, we show that \cref{centerproj} acts as the identity on $\C V^c(x)$. Since $\Psi^c(x) = W_0$ is a solution to the adjoint variational equation and $V^c(x)$ is solution to the variational equation, by Lemma \ref{eigadjoint}(i), the inner product $\langle \Psi^c(x), V^c(x) \rangle = \langle W_0, V^c(x) \rangle$ is constant. Sending $x \rightarrow \infty$ and using the continuity of the inner product, we have
\[
...
Next, we show that $W_0$ is perpendicular to any element of the stable and unstable subspaces at $x$. Let $\tilde{V}^s$ be any element of the stable subspace at $x$. Then $\tilde{V}^s = P_+(x, 0) V^s$ for some $V^s \in E^s(0)$. Let $V^s(y)$ be the unique solution to the variational equation with initial condition $V^s(x) = \tilde{V}^s$. This solution is given by
\begin{align*}
...
THIS DOES NOT WORK FOR THE UNSTABLE SUBSPACE SINCE WE CANNOT SEND $x \rightarrow -\infty$. A SYMMETRY ARGUMENT OR SOMESUCH IS NEEDED HERE.
\end{proof}
\end{lemma}
...
\] 
Using the Taylor series for $W_0(\lambda)$ from \cref{nulambdalemma} and the deacay rates \cref{Thetadecay} for the conjugation operators,
\begin{align*}

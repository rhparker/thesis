...
\end{enumerate}
The result follows from equation \eqref{inteigsformula} from Lemma \ref{inteigslemma}. The condition that $m_{n-1} \geq M_1$ is equivalent to $b_{n-1}^0 \leq \tilde{b}^*$ for some $\tilde{b}^* \leq b^*$.

\subsection{Interaction Eigenvalues: Specific Cases}

There are specific cases, however, in which we can compute the eigenvalues of $\tilde{A}$ in terms of the terms $\tilde{a}_j$.

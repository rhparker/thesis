...
$DF(0)$ is a $2m \times 2m$ constant coefficient matrix of the form
...
\end{pmatrix}
\end{equation}
where reversibility implies that the coefficients $c_i$ in the bottom row are all 0 for $i$ odd. The characteristic polynomial of $DF(0)$ is
...
\end{align*}
are eigenvectors of $A(0)$ and $-A(0)^*$ (respectively); we have scaled $V_0$ so that $\langle V_0, W_0 \rangle = 1$.
...
& & \ddots  \\
0 & 0 & \dots & 0 & 1 \\
f_0'(x) & -c + f_0(x) + f_1'(x) & \dots & c_{2m-2} + f_{2m-2}(x) + f_{2m-1}' (x) & f_{2m-1}(x)
\end{pmatrix}
\end{equation} 
By reversibility, since no odd-order derivatives are involved in the linear part of $E'(u)$, $c_i = 0$ for $i$ odd. In addition, since $q(x)$ is an even function, $f_i(x)$ is an even function for $i$ even and an odd function for $i$ odd. Since the functions $f_i(x)$ are smooth functions of $q(x)$ and its derivatives, they decay exponentially to 0 since $q(x)$ is exponentially localized.
...
[\partial_x H_c]^* v(x) = -H_c \partial_x v(x) = 0,
\end{equation}

To find $\Psi(x)$, equation \ref{adj2} has an exponentially decaying solution $q(x)$. Using Lemma \ref{adjointlemma}, \eqref{adjvareq2} has a solution $\Psi(x) = (\psi_1(x), \dots, \psi_{2m+1}(x))$ with $\psi_{2m+1}(x) = q(x)$. Comparing \eqref{Axform} from Lemma \ref{adjointlemma} to \eqref{DefAQ}, the functions $g_k(x)$ in \eqref{Axform} are even functions for $k$ odd and odd functions for $k$ even. Using \eqref{adjWform} from Lemma \ref{adjointlemma}, for the components of $\Psi(x)$ we have $\psi_k(x)$ is even for $k$ odd and odd for $k$ even, thus $\Psi(-x) = R \Psi(x)$, where $R$ is the standard reversor operator. Finally, since the functions $f_k(x)$ in \eqref{DefAQ} are exponentially localized, $\Psi(x)$ is exponentially localized as well.

To find $\Psi^c(x)$, equation \eqref{adj2} also has the constant solution 1 as a solution. Thus \eqref{adjvareq2} has a solution $\Psi^c(x) = (\psi^c_1(x), \dots, \psi^c_{2m+1}(x))$ with $\psi_{2m+1}(x) = 1$. Comparing \eqref{Axform} from Lemma \ref{adjointlemma} to \eqref{DefAQ} and using \eqref{adjWform} from Lemma \ref{adjointlemma}, we have
\[
\Psi^c(x) 
= \begin{pmatrix}
...
\Psi^c(x) 
= \begin{pmatrix}
c - f_0(x) - f_1'(x) + \sum_{k=2}^{n-1} (-1)^{k-1} \partial_x^{k-1} g_k  \\
...
\item For the terms involving $a$, we plug in $A_4$ from Lemma \ref{Zinv2}.
\begin{align*}
...
= \langle \Psi^c(0), \Theta^s(0, -X_{i-1}; 0) P_0^s(0) D_{i-1} d \rangle + \mathcal{O}(e^{-\alpha X_{i-1}}(|\lambda| + e^{-2 \alpha X_m})|d|) \\
&= \langle \Psi^c(0), \Theta(0, -X_{i-1}; 0) P^s_-(-X_{i-1}) P_0^s(0) D_{i-1} d \rangle + \mathcal{O}(e^{-\alpha X_{i-1}}(|\lambda| + e^{-2 \alpha X_m})|d|) \\
&= \langle \Psi^c(-X_{i-1}), P^s_-(-X_{i-1}) P_0^s(0) D_{i-1} d \rangle + \mathcal{O}(e^{-\alpha X_{i-1}}(|\lambda| + e^{-2 \alpha X_m})|d|)
\end{align*}
...
P^s_-(-X_{i-1}) P_0^s(0) Q'(-X_{i-1})
&= P^s_-(-X_{i-1}) Q'(-X_{i-1}) + \mathcal{O}(e^{-2 \alpha X_{i-1}}) \\
&= \mathcal{O}(e^{-2 \alpha X_{i-1}}) \\
\end{align*}
Combining these, the leading order term is
\begin{align*}
\langle \Psi^c(0), &\Theta_i^s(0, -X_{i-1}; \lambda) P_0^s(0) D_{i-1} d \rangle \\
&= \langle \Psi^c(-X_{i-1}), Q'(X_{i-1}) \rangle (d_i - d_{i-1} ) + \mathcal{O}(e^{-\alpha X_m}(|\lambda| + e^{-\alpha X_m})|d|)
\end{align*}
For the higher order term, we use the bound for $A_4$ from Lemma \ref{Zinv2} to get
\begin{align*}
\langle \Psi^c(0), &P_i^-(0; \lambda) \Phi^s(0, -X_{i-1}; \lambda) A_4(\lambda)_{i-1}^-(c, d) \rangle \\
...
&= \langle \Psi(0), P_i^-(0; \lambda) \Phi^s(0, -X_{i-1}; \lambda) (- P_i^-(-X_{i-1}; \lambda)^{-1} P_0^s(\lambda) D_{i-1} d + A_4(\lambda)_{i-1}^-(c^-, d)) \rangle \\

...
% epsilon balls lemma, with proof
...
\end{equation}
\item If the $\epsilon-$ball condition holds,
...
% lemma : parity of eigenvalues determines eigenvalue pattern
...
For bound (ii), suppose $|\lambda| \geq 2 r^{1/2} \tilde{\mu}_M$. For the pairs $\pm \mu_j$ in \eqref{detAlambda}, we have the lower bound
\begin{align*}
(\lambda - r^{1/2} \sqrt{\tilde{\mu}/M} )(\lambda + r^{1/2} \sqrt{\tilde{\mu}_1/M} )
...
START HERE. LAMBDA IS RELATED TO SQRT MU not MU.
...
To continue, we need to bound $(A - \lambda^2 MI)^{-1} D_2$, which we will do for the two cases in Lemma \ref{Ainvboundlemma}. For $|\lambda| \geq 2 r^{1/2} \tilde{\mu}_M$, we have
\begin{align*}
...
Next, we will obtain bounds on the terms in the above equation. For the term $D_1 (A - \lambda^2 MI)^{-1}$, we consider the same two cases we did above. For $|\lambda| \geq 2 r^{1/2} \tilde{\mu}_M$, we have
\begin{align*}
|| D_1 (A - \lambda^2 MI)^{-1} || &\leq \frac{C}{|\lambda|^2} (|\lambda| + r^{1/2})^2 \\ 
&\leq C
\end{align*}
For $|\lambda| \leq 2 r^{1/2} \tilde{\mu}_M$, we have
\begin{align*}
...
Take $\lambda$ with $|\lambda| = \delta$. For all $k$ such that $|\lambda^K(X, k)| \leq \delta$, this implies
\[
| \lambda - \lambda^K(X, k)| \geq C \frac{1}{X}
\]
Following the proof of Lemma \ref{detKlemma}, we have
\[
|\det K(\lambda)| \geq C
\]
Following the proof of \ref{Kinvboundslemma}, this implies
\[
||K(\lambda)^{-1}|| \leq C \\
\]

Since the $2n-2$ nonzero roots of $\det(A - \lambda^2 M I)$ are $\mathcal{O}(r^{1/2})$, we can find $r_3 \leq r_2$ such that for all $r \leq r_3$, $2 r^{1/2} \tilde{\mu}_M \leq \delta$, where $\tilde{\mu}_M = \max\{\tilde{\mu}_1, \dots, \tilde{\mu}_{n-1} \}$. Thus for $|\lambda| = \delta$, using Lemma \ref{Ainvboundlemma} we have the following bound for $A - \lambda^2 M I$
\begin{equation*}
...
A complete account of the eigenvalues inside $|\lambda| = \delta$ is as follows.
\begin{enumerate}

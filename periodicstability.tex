...
p_2(\nu) = -\nu^{2m+1} + c_{2m-2} \nu^{2m-1} + \dots + c_2 \nu^3+ c \nu = \nu p_1(\nu)
\end{equation}
where $p_1(\nu)$ is the characteristic polynomial of $DF(0)$ from the previous section. Thus $A(0)$ has the same eigenvalues as $DF(0)$ as well as an additional eigenvalue at 0. The eigenvalues of $DF(0)$ are characterized in Hypothesis \ref{hypeqhyp}.
...
\end{align*}
are eigenvectors of $A(0)$ and $-A(0)^*$ (respectively), and that $V_0$ is scaled so that $\langle V_0, W_0 \rangle = 1$.
...
For estimate (v), using the definition of $D_i d$ and estimate (i), we have

...
In this chapter, we look at the spectral stability of the periodic multi-pulses which we constructed in \cref{chapter:kdv5periodic}. To do this, we will adapt framework we set up in \cref{sec:genspectrum}.

Let $Q(x)$ be the primary pulse solution, and let $Q_n(x)$ be periodic multi-pulse solution constructed according to Theorem \ref{perexist}. Recall that
\[
Q_n(x) = \left(q_n(x), \partial_x q_n(x), \dots, \partial_x^{2m-1}q_n(x) \right)^T
\]
We are interested in the PDE eigenvalue problem
\begin{equation}\label{genPDEeigqn}
\partial_x \calL(q_n) v = \lambda v
\end{equation}
resulting from the linearization of the original PDE about $q_n(x)$. From the construction in Theorem \ref{perexist}, $Q_n(-X) = Q_n(X)$ and $Q_n(x)$ satisfies the equation $Q_n'(x) = F(Q_n(x); c)$. It follows that $\partial_x^k q_n(-X) = \partial_x^k q_n(X)$ for $k = 0, \dots, 2m$. Thus it is natural to pose \cref{genPDEeigqn} on the space $H^{2m}_{\text{per}}[-X,X]$, where
\[
H^{2m}_{\text{per}}[-X,X] = \left\{ f \in H^{2m}(\R) : f^{(k)}(-X) = f^{(k)}(X) \text{ for } k = 0, \dots, 2m \right\} 
\]
The norm on this space has the same form as the norm on $H^{2m}$ with the integrals restricted to $[-X, X]$. 

As in \cref{sec:genspectrum}, we will write the eigenvalue problem as a first order system of ODEs. This time, however, we impose an additional periodic boundary condition. Thus we will consider the problem
\begin{equation}\label{PDEeigsystemper}
V'(x) = A(Q_n(x))V(x) + \lambda B V(x),
\end{equation}
where $V(x) \in C^0_{\text{per}}[-X,X]$. This is equivalent to the pair of equations
...
We will use Lin's method to construct eigenfunctions $V(x)$ as piecewise perturbations of the kernel eigenfunctions $\partial_x Q_n(x)$ and $\partial_c Q_n(x)$. We will make this specific in the next section. As long as the individual pulses are well-separated, Lin's method produces a unique function $V(x)$ which solves \cref{PDEeigsystemper2} but which generically has $n$ jumps. These $n$ jumps are in the two-dimensional space spanned by $\Psi(0)$ and $\Psi^c(0)$, which gives us $2n$ jump conditions. Finding the eigenvalues amounts to solving the $2n$ jump conditions. As in \cite[Theorem 2]{Sandstede1998}, we will write these jump conditions in matrix form. In this case, we will have a $2n \times 2n$ block matrix where the diagonal blocks represent, to leading order, interaction eigenvalues and essential spectrum eigenvalues. We will then use this block matrix equation to actually locate these eigenvalues, first for the periodic 2-pulse and then for general periodic multi-pulses.

\section{Setup of problem}

First, we charcterize the kernel eigenfunctions. When $\lambda = 0$, it follows from \cref{sec:HamPDE} and the definitions of $A(Q_n(x))$ and $B$ that 
...
\end{aligned}
\end{equation}
By the construction of $Q_n(x)$ in Theorem \ref{perexist}, both $\partial_x Q_n(x)$ and $\partial_c Q_n(x)$ satisfy the periodic boundary conditions.

In the next lemma, we show that there is an additional kernel eigenfunction which corresponds to $V^c(x)$.

\begin{lemma}\label{lemma:centereigenfn}
There exists a function $V_n^c(x)$ which satisfies
\begin{align*}
...
\calL(q_n) v = 1
\end{equation}
posed on $H^{2m}_{\text{per}}[-X,X]$. Since both $q_n$ and the constant function 1 are in $H^{2m}_{\text{per}}[-X,X]$, this is well-defined. The operator $\calL(q_n)$ is self-adjoint and has kernel spanned by $\partial_x q_n$. We can show that
\[
\langle \partial_x q_n(x), 1 \rangle_{H^{2m}_{\text{per}}[-X,X]} = 0
\]
by integrating by parts; the boundary term cancels due to the periodic boundary conditions. Since $1 \perp \ker \calL(q_n)$, it follows from the Fredholm alternative that \cref{Lqn1} has a solution $v(x) \in {H^{2m}_{\text{per}}[-X,X]}$. Let
\[
...
Next, we rewrite the eigenvalue problem \cref{PDEeigsystemper} as
...
Assume Hypotheses \ref{Ehyp}, \ref{Hhyp}, \ref{hypeqhyp}, \ref{Qexistshyp}, and \ref{H0transversehyp}. Let $Q(x)$ be the primary pulse solution, and let $Q_n(x)$ be a periodic $n-$pulse solution with pulse lengths $X_0, \dots, X_{n-1}$. Then there exists $\delta > 0$ with the following property. There exists a bounded, nonzero solution $V$ of \eqref{PDEeig3} for $|\lambda| < \delta$ if and only if the $(2n \times 2n)$ block matrix equation
...
While Theorem \ref{blockmatrixtheorem} reduces the eigenvalue problem to finding the determinant of a $2n \times 2n$ matrix, that matrix contains an unwieldy amount of remainder terms. Since our goal was to derive equation \cref{blockeq} with as few assumptions as possible, the complicated form of the remainder matrices is a consequence of the fact that we do not have a uniform bound for terms of the form $e^{\nu(\lambda)X_j}$. 

The following theorem gives a more convenient version of Theorem \ref{blockmatrixtheorem}. To derive the theorem, we will use the parameterization of the perioidic multi-pulse from Theorem \ref{perexist}. We will then simplify the analysis by imposing a condition on the scaling parameter $r$. This condition, together with another condition on the real part of $\lambda$, will allow us to uniformly bound terms of the form $e^{\nu(\lambda)X_j}$ and obtain a much cleaner result.

% block matrix theorem
\begin{theorem}\label{blockmatrixtheorem2}
Assume Hypotheses \ref{Ehyp}, \ref{Hhyp}, \ref{hypeqhyp}, \ref{Qexistshyp}, and \ref{H0transversehyp}. Let $Q(x)$ be the primary pulse solution. Choose $n \geq 2$, baseline length parameters $\{b_0^0, \dots b_{n-1}^0\}$ and phase parameter $\theta$. Let $Q_n(x; r)$ be the family of periodic multi-pulse solutions corresponding to our choice of parameters, where $r \in \mathcal{R}$ with $r \leq r_*$.

Choose any $R_1 > 0$ and $R_2 > 0$, and let $r_0 \leq r_*$ be sufficiently small so that
\begin{equation}\label{simplecondition}
n|\log r_0|\:r_0^{1/2} \leq R_1
\end{equation}
Then there exists $\delta > 0$ with the following property. For all $r \leq r_0$ with $r^{1/2} < \delta$, there exists a bounded, nonzero solution $V$ of \eqref{PDEeig3} for $|\lambda| < \delta$ and $|\Re \lambda| < R_2 r^{1/2}$ if and only if the $(2n \times 2n)$ block matrix equation
\begin{equation}\label{blockeq}
\begin{pmatrix}
K(\lambda) + C_1 & \lambda S(\lambda) + D_1 \\
-\lambda M^c \tilde{K}(\lambda) + C_2 & A - \lambda^2 MI + D_2
\end{pmatrix}
\begin{pmatrix} c \\ d \end{pmatrix} = 0
\end{equation}
has a nontrivial solution. The individual terms in the block matrix are as follows.
...
\end{align*}
The constants $C$ depend on the choices of $R_1$ and $R_2$.
\end{enumerate}
\end{theorem}
...
Assume Hypotheses \ref{Ehyp}, \ref{Hhyp}, \ref{hypeqhyp}, \ref{Qexistshyp}, \ref{H0transversehyp}, \ref{Melnikov2hyp}, and \ref{Adistincteigs}. Choose $n \geq 2$, baseline length parameters $\{b_0^0, \dots b_{n-1}^0\}$ and phase parameter $\theta$. Let $Q_n(x; r)$ be the family of periodic multi-pulse solutions corresponding to our choice of parameters, where $r \in \mathcal{R}$ with $r \leq r_*$. Let $\{ b_0(r), \dots, b_{n-1}(r)\}$ be the length parameters associated with $Q_n(x; r)$.

Let $\delta > 0$ and $M$ be as in Theorem \ref{blockmatrixtheorem2}. Let $A$ be the matrix in the lower right block of \cref{blockeq2}, and take $A = r \tilde{A}(r)$, where we have made the dependence on $r$ explicit. Let $(0, \ell_1(r), \dots, \ell_{n-1}(r))$ be the distinct eigenvalues of $\tilde{A}(r)$. Let
\begin{equation}
...
\begin{enumerate}
	...
	\item Choose any $\eta > 0$ such that $2 \eta < \alpha_1$.

	\item Let
	\begin{align*}
	\alpha &= \alpha_1 - \eta \\
	\tau &= \frac{\alpha}{\alpha_1}
	\end{align*}
	By our choice of $\eta$
	\[
	\frac{1}{2} < \tau < 1
	\]
...
To conclude this section, we will collect estimates on the terms involved in \eqref{eigsystem} in the following two lemmas.
...
In the next lemma, we give an estimate for terms of the form $e^{\pm \nu(\lambda) X_j}$, which will show up in our analysis. This requires placing an upper bound one magnitude of $\lambda$.

\begin{lemma}\label{lemma:expnlbound}
For any periodic parameterization $(b_0, \dots, b_{n-1}, \theta)$, let $r_*$ be as in Theorem \ref{perexist}. Then for any $N > 0$ and $\delta > 0$, there exists a constant $R_N$ such that if $|\lambda| < \delta$ and $|\lambda| < N/|\log r|$,
\begin{align}\label{expnlbound}
|e^{\nu(\lambda)X_j (r; m_j, \theta)}| \leq R_N && j = 0, \dots, n-1
\end{align}
\begin{proof}
Using the expression \cref{Xj} from \cref{perexist},
\begin{align}\label{Xjmag}
|X_j(r; m_j, \theta)| &\leq \frac{1}{2 \alpha_0} |\log r| + \frac{\pi}{2\beta_0} |b_j(r; m_j, \theta)| + |L_0| && j = 0, \dots, n-1,
\end{align}
For all $r \leq r_^*$ and $\theta$, the functions $b_j(r; m_j, \theta)$ are smooth in $r$ and satisfy bound $|b_j(r; m_j, \theta)| \leq m_j + 1$. Let $m_{\max} = \max\{ m_0, \dots, m_{n-1}\}$. Then we have the uniform bound
\[
|b_j(r; m_j, \theta)| \leq m_{\max} + 1
\]
Combining this with \cref{Xjmag}, we have the uniform bound
\begin{align}\label{Xjmag2}
|X_j(r; m_j, \theta)| &\leq \frac{1}{2 \alpha_0} |\log r| + \frac{\pi}{2\beta_0} (m_{\max} + 1) + |L_0| && j = 0, \dots, n-1,
\end{align}
...
\end{align*}
which is bounded for all $r \leq r_*$. Thus we conclude that there exists a constant $R_N$ depending on $N$ such that for all $j = 0, \dots, n-1,$
\[
|e^{\nu(\lambda)X_j(r; m_j, \theta)}| \leq R_N
\]
\end{proof}
\end{lemma}

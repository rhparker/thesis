...
Rearranging \cref{nobubblecondgen}, there exists a constant $R_1$ such that $n|\log r_1|\:r_1^{1/2} \leq R_1$. Furthermore, we will only need to consider eigenvalues $|\lambda|$ with real part smaller than $2 r^{1/2} \sqrt{R_{\max}/M}$, thus there exists a constant $R_2$ for which $|\Re \lambda| < R_2 r^{1/2}$. Since these conditions are satisfied, we can use the form of the block matrix in Theorem \ref{blockmatrixtheorem2}.

The outline of the proof is as follows. To find the interaction eigenvalues, we will solve the first line of the block matrix equation \cref{blockeq2} for $c$ and substitute the result into the second line. To find the essential spectrum eigenvalues, we will solve the second line of the block matrix equation \cref{blockeq2} for $d$ and substitute the result into the first line. These steps require inverting the matrices $K(\lambda)$ and $A - \lambda^2 M I$; the condition \cref{nobubblecondgen} assures that this can be done and gives us sufficient bounds on their inverses. For the eigenvalue counts, we will use Rouch\'e's theorem on an appropriate circle around the origin in the complex plane.

\subsection{Rescaling and simplification}

We will start with the simplified block matrix equation \cref{blockeq2} from Theorem \ref{blockmatrixtheorem2}. In the next lemma, we simplify further by combining the off-diagonal blocks into a single remainder term, making the change of variables $\lambda = \nu^{-1}(\mu)$ to eliminate the $\nu(\lambda)$ terms, and rescaling the matrix $A$.

% block matrix reparameterization
\begin{lemma}\label{reparam}
The block matrix equation \cref{blockeq2} simplifies to
...
\begin{aligned}
S_1 &= \mathcal{O}(|\mu|(|\mu| + r^{1/2})) \\
S_2 &= \mathcal{O}(|\mu|(|\mu| + r^{1/2})) \\
S_3 &= \mathcal{O}(|\mu| + r^{1/2}) \\
S_4 &= \mathcal{O}((|\mu| + r^{1/2})^3) 
\end{aligned}
\end{equation}
\begin{proof}
First, we simplify the remainder matrices. Using \cref{lemma:expnubound}, we can bound all of the terms of the form $e^{\pm \nu(\lambda)X_j}$ in $S(\lambda)$ and $\tilde{K}(\lambda)$ by a constant. Thus we have $S(\lambda) = \mathcal{O}(r^{1/2})$ and $\tilde{K}(\lambda) = \mathcal{O}(1)$. Using these bounds to combine the off-diagonal matrices into a single remainder matrix, equation \cref{blockeq2} becomes
...
\begin{pmatrix} c \\ d \end{pmatrix} = 0
\end{equation}
and the remainder matrices $S_j$ have the bounds
\begin{equation}\label{remSlambdabounds}
\begin{aligned}
S_1 &= \mathcal{O}(|\lambda|(|\lambda| + r^{1/2})) \\
S_2 &= \mathcal{O}(|\lambda|(|\lambda| + r^{1/2})) \\
S_3 &= \mathcal{O}(|\lambda| + r^{1/2}) \\
S_4 &= \mathcal{O}((|\lambda| + r^{1/2})^3) 
\end{aligned}
\end{equation}
...
\begin{pmatrix} c \\ d \end{pmatrix} = 0
\end{equation}
The remainder matrices have the bounds \cref{remSbounds}, which are the same as \cref{remSlambdabounds} with $\lambda$ replaced with $\mu$.

Finally, using \cref{lemma:ajparam}, each term $a_j$ in the matrix $A$ is of the form $a_j = r \tilde{a}_j$, thus we can write the matrix $A$ as $A = r \tilde{A}$. Making this last substitution, we obtain equation \cref{blockeq3}.
\end{proof}
\end{lemma}
...
To find the essential spectrum eigenvalues, we solve the second line of of the block matrix equation \eqref{blockeq3} for $d$ and plug it into the first line. We will always assume that \cref{mucondess} holds. In the next lemma, we derive an equation we can solve to obtain the essential spectrum eigenvalues.
